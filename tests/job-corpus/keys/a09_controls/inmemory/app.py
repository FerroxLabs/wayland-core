"""Negative control for A-9: correct in every way except durability.

A plausible scaffold that boots, serves, redirects, counts hits and refuses bad
input -- but keeps everything in a single in-process database that dies with
the process. The probe must FAIL this on the restart checks and on nothing
else. If it passes, A-9 is not testing durability at all.
"""
