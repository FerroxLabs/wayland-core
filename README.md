...
- **Fail-closed network policy.** `BrowserPolicy` runs pre-dispatch on every URL-bearing op, `default_action = Deny` since v0.2.1. Scheme allowlist is http/https only. Hardcoded blocks — regardless of your allow/deny lists — cover RFC1918 ranges, loopback, the `169.254.169.254` metadata endpoint, link-local, IPv6 ULA, and legacy IPv4 encodings (octal, hex, decimal). Every navigation host is also re-checked after DNS resolution — a name that resolves to a blocked address is refused, an unresolvable one fails closed, and the surviving answer is pinned in a TOFU cache so a later navigation cannot swap it. Redirects on requests Core issues itself are re-evaluated per hop, capped at 10; redirects the sidecar follows internally are caught by re-checking the landing URL afterwards. The sidecar resolves DNS in its own process, so a TTL=0 record can still rebind inside a single navigation — that gap is open by design, not closed.
