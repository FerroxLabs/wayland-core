...
- **Default-deny permissions, once a gate is installed.** A multi-actor policy engine gates every (actor, resource, action) tuple it is asked about: no matching grant means denied. The gate is opt-in rather than default-on, so a session that never installs one runs its tools with no ACL enforcement; the layers that are active in every session regardless are the egress gate, the OS sandbox, the approval modes and the budget caps. File globs reject any `..` segment before matching, and bearer tokens are SHA-256 signed with a TTL, revocation, and rotation that cannot extend an expiry.
...
- **Multi-backend, runtime-selected.** Camoufox is the default and talks to a sidecar over HTTP; Browserbase (cloud) is opt-in behind a cargo feature. The default build is Camoufox-only. CUA picks its backend from the platform: CGEvent on macOS, x11rb XTest on Linux X11 (on by default), `wlrctl` + `grim` on Wayland (opt-in), UI Automation + SendInput on Windows. An unsupported target returns a typed error, never a silent no-op.
- **Fail-closed network policy.** `BrowserPolicy` runs pre-dispatch on every URL-bearing op, `default_action = Deny` since v0.2.1. Scheme allowlist is http/https only. Hardcoded blocks — regardless of your allow/deny lists — cover RFC1918 ranges, loopback, the `169.254.169.254` metadata endpoint, link-local, IPv6 ULA, and legacy IPv4 encodings (octal, hex, decimal). Every navigation host is also re-checked after DNS resolution — a name that resolves to a blocked address is refused, and an unresolvable one fails closed. Redirects on requests Core issues itself are re-evaluated per hop, capped at 10; redirects the sidecar follows internally are caught by re-checking the landing URL afterwards. The sidecar resolves DNS in its own process, so Core dials for it: a Camoufox sidecar Core launches is pointed at a loopback proxy Core runs, which screens every connection the browser opens and connects to the address it screened — closing TTL=0 intra-navigation rebinding and bringing sub-resource loads under the same address gate. A sidecar Core did *not* launch cannot be contained, so it is refused, with a named opt-out (`allow_unproxied_sidecar`) that spells out what turning it on gives up. Loopback is included: Firefox would dial `127.0.0.1` and `localhost` around any configured proxy, so Core writes `network.proxy.allow_hijacking_localhost` into the Camoufox install before starting the sidecar and refuses to start one when it cannot — a loaded page's own requests to this machine hit the same gate, refused unless a port-scoped loopback grant authorises them.
