...
- **A live test proves nothing if its scenario is too clean to reach the defect — the scenario must carry state.** Live-testing is necessary and it is not sufficient. Measured: `gateway drain` returned the per-iteration increment where its contract requires total elapsed, so it hung indefinitely whenever work was carried. It passed the **first** live journey, because that gateway had zero pending deliveries and the drain loop broke on its first observation. A green suite, green clippy and a green *live* run all missed it; it surfaced only on a second journey that carried 12 deliveries across a `kill -9`. So when you design a live exercise, ask what state the code path needs in order to be *wrong*, and put that state there — an empty queue, a fresh profile, a single worker and a zero-length history are all scenarios in which broken code behaves correctly.
