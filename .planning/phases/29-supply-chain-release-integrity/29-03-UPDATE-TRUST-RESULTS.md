...
| **F29-03-08** | **MEDIUM** | This plan's FENCE and SURGICAL-DIFF gates use `git status --porcelain`, which reports only **uncommitted** changes. The plan commits per task, so by the time either gate runs the tree is clean and it passes regardless of what was changed — the "already green, proves nothing" class. | Re-run in the non-vacuous merge-base form against the SHA captured once at lane start: **0 fenced files touched, 6 `crates/` paths touched, all whitelisted, 0 off-whitelist.** Both forms in `evidence/29-03/local-gates.txt`. **SR-29-12**. |
