...
must_haves:
  ...
  artifacts:
    ...
    - path: scripts/wayland-e2e-windows-soak.ps1
      provides: "The ten previously unselected `live_fs_acl` ACL security tests and the orphaned containment gate marker wired into a NON-PROOF runner, leaving the six-target proof script's target array untouched"
...
<task type="auto">
  ...
  <files>.github/workflows/ci.yml, .github/workflows/nightly-windows-soak.yml, scripts/wayland-e2e-windows-soak.ps1</files>
  <read_first>scripts/wayland-e2e-windows-soak.ps1 (the phase that invokes nextest over the five crates, and its surrounding phase structure), scripts/f20-native-uat-proof.mjs (`verifyNativeLog` and the canonical Windows target list — read this to understand WHY the proof script's target array is off limits), scripts/f20-native-windows-proof.ps1 (READ ONLY — the target array, the live-acceptance environment variable and the wrong-OS anti-drift guard; this file is NOT modified by this plan), .github/workflows/ci.yml (the `on:` block and the self-hosted Windows matrix entry), crates/wcore-sandbox/tests/live_fs_acl.rs (the live-acceptance gate helper at the top of the file and all twelve ignored test names)</read_first>
  <behavior>
    ...
    - The six-target native proof script's target array is BYTE-IDENTICAL after this plan as before it. Nothing is added to it, nothing is removed from it, and no selector filter inside it is widened — the native proof's fail-closed marker verification depends on that array being exactly the canonical six.
...
WIRING C — give the ten orphaned ACL security tests a runner, and DO IT ANYWHERE EXCEPT THE NATIVE PROOF SCRIPT. Twelve Windows-only ignored tests live in `crates/wcore-sandbox/tests/live_fs_acl.rs`; the six-target proof script names two of them and MUST GO ON NAMING EXACTLY THOSE TWO. Wire all twelve on the SOAK surface — the same script Wiring B just extended — or on a separate non-proof runner. Prefer the smallest change that makes the whole file's ignored set selectable rather than enumerating names one by one: an enumerated list is exactly how ten of them fell out, and a file-level selection cannot silently lose a test that is added later. Whichever runner you choose, it must run the ignored set and it must fail closed if a selector matches nothing, matching the discipline the proof script already applies. The live acceptance helper at the top of that file gates on an environment variable: set it using the trap-safe form and PROVE it took effect before trusting the run, because the trailing-space form silently disables every one of these tests and would produce a vacuous green. Apply the same treatment, on the same non-proof runner, to the `hard_process_containment_windows` gate marker that no gate currently selects. Do NOT delete it — the audit's "FIX or DELETE" is resolved as FIX, because deleting a test to tidy a gate is precisely the move this phase forbids.

WHY WIRING C IS FENCED OFF THE PROOF SCRIPT (this is not stylistic — it is a correctness constraint). `verifyNativeLog` in `scripts/f20-native-uat-proof.mjs` requires that every target marker in a native job log be one of the six canonical Windows targets, appear EXACTLY ONCE, and appear IN ORDER; a marker outside that set fails closed as a foreign target marker, and a repeat fails closed as a duplicate. So adding entries to the proof script's target array would make the six-target proof fail closed on its own new markers. The only other route — widening the selector filters on `windows-retained-handle` or `windows-appcontainer-acl` — would load two CERTIFIED-GREEN targets with ten tests that have never executed anywhere, and very likely turn them red. Either route makes Phase 20A Success Criterion 1 unreachable. The proof script's target array is 20A-04's verified invariant, not this plan's surface.
...
CAP ON macOS COMPILE-REPAIR ROUND-TRIPS (M5): the macOS-only compile verdict can only come from a CI round-trip, and each repair costs another one. At most TWO round-trips. If the macOS-only bodies still do not compile after the second, STOP — record the diagnostics and escalate to Sean as compile-blocked. Do not iterate a third time.
...
Run the newly wired surface on the box and record the outcome per test. Tests that go RED here are the plan working as intended: name them, classify them, route them. Do not fix them — behavioural repair is 20A-02's and 20A-03's scope. Records evidence for REQ-native-r1, r2, r5, r7 and r8; marks no requirement complete.</action>
  <verify>
    ...
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; /usr/bin/git diff --exit-code -- scripts/f20-native-windows-proof.ps1 &amp;&amp; echo "proof script target array is 20A-04's invariant and is unmodified"</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; N=0; for t in ungranted_path_is_denied_in_sandbox granted_path_is_readable_then_revoked denied_secret_under_granted_parent_is_unreadable_and_revoked one_exit_does_not_remove_another_execution_grant one_execution_grant_never_leaks_to_another_identity concurrent_allow_and_deny_identities_do_not_interfere twenty_concurrent_executions_have_unique_temp_roots timeout_and_cancellation_remove_their_leases unrelated_acl_survives_exact_sid_cleanup deny_ace_still_blocks_granted_read normal_sid_only_grant_is_denied native_acceptance_gate_marker; do /usr/bin/grep -qF "$t" crates/wcore-sandbox/tests/live_fs_acl.rs &amp;&amp; N=$((N+1)); done; test "$N" = "12" &amp;&amp; echo "all twelve ACL test names still present in source: $N"</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; test "$(/usr/bin/grep -cF 'live_fs_acl' scripts/wayland-e2e-windows-soak.ps1)" -ge "1" &amp;&amp; test "$(/usr/bin/grep -cF 'run-ignored' scripts/wayland-e2e-windows-soak.ps1)" -ge "1" &amp;&amp; echo "the ACL ignored set is selected on the soak (non-proof) runner"</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; test "$(/usr/bin/grep -cF 'hard_process_containment_windows' scripts/wayland-e2e-windows-soak.ps1)" -ge "1" &amp;&amp; echo "the orphaned containment gate marker is selected on the non-proof runner"</automated>
    ...
    <automated>ssh -o BatchMode=yes SeanD@seandesktop 'powershell -NoProfile -Command { Set-Location C:\ferrox-win; $env:WAYLAND_SANDBOX_LIVE_WINDOWS='"'"'1'"'"'; cmd /c "cargo nextest run -p wcore-sandbox --test hard_process_containment_windows --run-ignored all --no-fail-fast 2>&amp;1" }' | grep -v CLIXML | grep -v "^&lt;Objs" | tail -30</automated>
  ...
  <done>CI fires on this branch and has produced at least one run whose macOS leg compile outcome is recorded, within at most two compile-repair round-trips. `wcore-sandbox` is in the recurring Windows soak's crate list and all five original crates survive. All twelve `live_fs_acl` ACL tests and the previously unselected containment gate marker are selected by a NON-PROOF runner that executes, with the live-acceptance environment variable set in the trap-safe form and PROVEN to have taken effect. `scripts/f20-native-windows-proof.ps1` shows NO diff — its target array is 20A-04's verified invariant and this plan did not touch it. The wrong-OS anti-drift guard is intact and no target was reclassified. Every newly wired test's outcome is recorded by name. No test was deleted, re-gated, `#[ignore]`d or `#[allow]`ed.</done>
</task>
...
- **`scripts/f20-native-windows-proof.ps1` is NOT modified — its target array is 20A-04's verified invariant.** The native proof's marker verification (`verifyNativeLog` in `scripts/f20-native-uat-proof.mjs`) fails closed on any target marker outside the canonical six, on any duplicate, and on any reordering. Adding a target there would make the six-target proof fail closed on its own new markers; widening the selector on `windows-retained-handle` or `windows-appcontainer-acl` would load two certified-green targets with ten never-executed tests. Either would make Success Criterion 1 unreachable. Wiring C therefore runs on the soak or another non-proof runner, and this file is gate-checked for a zero diff.
...
| T-20A-01-11 | Denial of Service | Wiring the orphaned ACL tests into the six-target native proof script breaks the proof itself: `verifyNativeLog` fails closed on a foreign or duplicate target marker, and widening a certified-green target's selector loads it with ten never-executed tests — either way Success Criterion 1 becomes unreachable | critical | mitigate | Wiring C is fenced entirely off `scripts/f20-native-windows-proof.ps1` and lands on the soak or another non-proof runner; the proof script is gate-checked for a ZERO DIFF in this plan, and 20A-04 Task 1 re-gates its target array for exactly six entries in canonical order before sealing |
...
<verification>
Local gates (Mac, source level only — the Mac cannot compile): `cargo fmt --all -- --check` clean; `scripts/wayland-e2e-windows-soak.ps1` names `wcore-sandbox` and still names all five original crates, and selects the whole `live_fs_acl` ignored set plus the previously unselected containment gate marker; `scripts/f20-native-windows-proof.ps1` shows a ZERO DIFF; all twelve ACL test names are still present in source; `ci.yml` carries a trigger that covers this branch; the OS-specific target count in the proof script is recorded and unreduced; `20A-01-BASELINE.md` exists and carries the pinned SHA, the compile verdicts and a severity column.
...
BACKLOG note for this phase (record, do NOT act on): **M6 — self-hosted CI contention.** Wiring A makes `ci.yml` fire on this branch, and its Windows leg runs on the SAME self-hosted box (`[self-hosted, Windows, X64, msvc]`) that Task 3 measures on and that 20A-02, 20A-03 and 20A-04 all use. A CI run triggered by a push can therefore compete with a measurement in progress for the box, the cargo target directory and any live-sandbox resource. Nothing in this phase serializes them. This is logged as non-blocking; if a measurement produces an inexplicable result, contention is the first hypothesis to check before diagnosing the code.
</verification>
...
- All twelve `live_fs_acl` ACL tests and the previously unselected containment gate marker are selected by a NON-PROOF runner that executes, with the live-acceptance flag proven effective rather than assumed (REQ-native-r1, r2, r5) — and `scripts/f20-native-windows-proof.ps1` shows a zero diff, so the six-target proof's fail-closed marker verification stays reachable for 20A-04.
...
- `scripts/wayland-e2e-windows-soak.ps1` and `.github/workflows/ci.yml` — the three wirings that end the blind spot, all landed off the six-target proof script.
