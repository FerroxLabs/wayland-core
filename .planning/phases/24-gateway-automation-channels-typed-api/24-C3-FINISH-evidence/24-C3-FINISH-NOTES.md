...
## T+3h — P3/P4 RESULT, and a new HIGH. Final state.

Full write-up in `../24-C3-FINISH.md`. Condensed running record:

- **run 1** — every submit `404`. MY instrument: invented route
  `/channels/:name/slack/events`; the real one is `POST /webhooks/:channel`
  (`inbound_webhook.rs:12-15`). Driver graded it FAIL, i.e. reported my own wrong URL as
  product inbound loss. Repaired + added `accepted`/`httpStatus` so a never-received message
  and a received-then-dropped message stop producing the same number.
- **run 2** — product path flawless (submit 200, one turn carrying the exact token, one reply
  at the sink) and the driver STILL graded FAIL: token `f24c3fin-…` does not match the
  fixture's `/f24c3-[a-z0-9-]+/i` (`f24-llm-fixture.mjs:88-91`), so it echoed
  `no-correlation`. Conformed to the fixture rather than editing it; the durable repair is
  the self-test assertion that reads the regex back out of the fixture source.
- **run 3 (reload)** — **FAIL 7/8**, `instrument_fault=false`, arrivals=2 turns=2.
  health PASS ×2, reload-registers PASS (2→3), unchanged-adapter-survives PASS `tier=exact`.
  **`reloaded-adapter-actually-carries-inbound` FAIL**: `inbound denied channel=f24finthree
  reason=sender not in dm allowlist`, `policies=2` at startup and never republished.
- **control (`--all-at-start`)** — **PASS 5/5**. The IDENTICAL config, generated by the
  IDENTICAL function, is ADMITTED when present at startup: `tier=exact`, arrivals 1→2,
  `configured=3 registered=3`. **One variable changed. H2 (bad config) excluded.**

**F24-C3-H5, HIGH, measured and NOT fixed:** `channel reload` registers the adapter and
reports it `healthy`, but `gateway.rs:1101-1157` never refreshes `inbound_host`, whose policy
map is captured once at `channel_inbound_host.rs:151` and moved into the spawned task at
`channel_inbound.rs:214`. Absent channel → `unwrap_or_default()` → `InboundPolicy::default()`,
which is fail-closed with an EMPTY allowlist (`access.rs:109-117`). Fails CLOSED — correct
posture, not a security hole — but three surfaces simultaneously tell the operator it works.

Not fixed because the repair has **two** facets: the policy map AND the tool postures
(`channel_inbound_host.rs:154-171`), which go stale by the same path. Fixing only the first
would pass a naive re-run while the new channel ran under a default posture — and "the repair
was PARTIAL, and the live run caught it" is a recorded lesson from this criterion one lane ago.

**Answers to the four T+0 open questions:**
1. matrix + signal: seams ALREADY EXIST, zero Rust. msteams: discord-shaped, 1–2 fields.
   imessage: blocked on platform, not on a seam.
2. Email reply half: REACHABLE via `add_root_certificate` (`tls.rs:248`, applied
   unconditionally at `tls.rs:518-526`), ~1 session. NOT from configuration.
   `dangerous_accept_invalid_certs` refused.
3. `gateway run` still receives: **YES** — `tier=exact` in 3000 ms on both graded runs.
4. health = closed on Linux; reload = PARTIAL; media + native actions = still zero.

Self-test 20/20 on macOS and Linux; proven able to fail (11/1 under mutation, only the
intended assertion reddening). Zero Rust files changed; fence empty vs `8bcb052b`.

**24-C3 remains NOT MET. I do not claim it.** Remaining distance ~6–8 lane-sessions on Linux
alone, before macOS and Windows.
