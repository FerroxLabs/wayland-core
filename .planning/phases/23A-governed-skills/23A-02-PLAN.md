...
files_modified:
  ...
  - Cargo.lock
  - crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs
  - scripts/f23a-promotion-drive.sh
  - scripts/f23a-promotion-drive.ps1
  - .planning/phases/23A-governed-skills/23A-02-BASE-SHA
...
must_haves:
  truths:
    ...
    - "THE LIVE DRIVER'S MECHANISM IS 23A-01'S, INHERITED AND NOT REINVENTED. 23A-01 established that nothing in `scripts/` can drive `wayland-core` against a scripted provider — `scripts/wayland-e2e-real-workload.sh` reaches only `--version` (line 327) and `--help` (line 329) — and that the working harness is Rust-side: `OpenAiFixtureScript` (`crates/wcore-eval-scenarios/src/fixtures/openai.rs:34`, `tool_call` at `:105`, `base_url` at `:407`) behind `run_with_binary_in_paths` (`crates/wcore-eval-scenarios/src/runner.rs:462`), which spawns the real binary as `--json-stream --provider <id> --model <model> --base-url <fixture>` (`:340` and `:346`), with the draft produced by the PRODUCT's own drafting path (`packaged_driver_gate.rs:691`, `:732`, `:749`). 23A-01 extracted that into `wcore_eval_scenarios::governed_skill_drive`. This plan REUSES that module: it adds `crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs` as its drive target and makes `scripts/f23a-promotion-drive.{sh,ps1}` thin wrappers whose exit status is `cargo test -p wcore-eval-scenarios --features packaged-driver-gate --test f23a_promotion_drive`, set up as `justfile:165-181` and `justfile:183-184` set up the packaged gate. Operator verbs — review, promote — are separate `wayland-core` process invocations with argv, the way `packaged_driver_gate.rs:267` already does it. Do not fork the harness, do not hand-seed a draft, and do not degrade this leg to the packaged gate."
    - "A REPORTED RED IS WORTH FAR MORE THAN AN ENGINEERED GREEN, AND A GATE THAT EXITS 0 UNCONDITIONALLY IS AN ENGINEERED GREEN WEARING A DIAGNOSTIC'S CLOTHES. Never weaken an assertion, add an ignore or allow attribute, raise a timeout, re-gate, or delete an inconvenient test to reach a gate. Never widen a policy threshold after seeing it fail. And never leave `git status --porcelain`, `git diff --stat` or `git diff --name-only` as a bare gate: all three exit 0 whether or not anything changed. Every untouched-surface and blast-radius claim here is `test -z \"$(...)\"` over a working-tree leg AND a base-SHA diff leg, because the working-tree leg alone goes permanently green the moment the executor commits. Findings at CRITICAL or HIGH must be fixed or disproved; MEDIUM and below go to `.planning/BACKLOG.md` and DO NOT BLOCK."
  artifacts:
    ...
    - path: crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs
      provides: "The Rust live-drive target the shell wrappers execute, built on 23A-01's `governed_skill_drive` module: an OpenAiFixtureScript provider, the real binary through --json-stream, the product's own drafting path, and the review/promote verbs invoked as separate wayland-core processes"
    - path: scripts/f23a-promotion-drive.sh
      provides: "The Linux live wrapper proving the same draft 23A-01 saw refused now executes after governed promotion, and that an unreviewed sibling still does not — SHA-asserting (exit 3 on mismatch), then exiting with the status of `cargo test ... --test f23a_promotion_drive`"
...
**THE BASE SHA (hard, and it is this plan's FIRST action).** Before reading or writing anything else, record the untouched starting commit: `/usr/bin/git rev-parse HEAD > .planning/phases/23A-governed-skills/23A-02-BASE-SHA`. Every untouched-surface and lock-drift gate reads it. Never rewrite it later — a base SHA that advances with the work turns every blast-radius gate green by construction.

**DRIVER SELF-TESTS (hard).** Both wrappers honour two externally-triggered self-tests, implemented once in the Rust drive target so both platforms inherit them. `WAYLAND_EXPECT_SHA` set to a SHA the checkout does not match MUST exit exactly 3 — a pinned code, because "distinct nonzero" is not gateable. `WAYLAND_F23A_SELFTEST=unreviewed` MUST make the drive target inject the failure its unreviewed control exists to catch — promote the sibling WITHOUT recording a review and then assert it executes — print `F23A-SELFTEST-TRIPPED: unreviewed`, and exit nonzero. A driver that stays green under that injection has a control that never fires.
...
Implements the evaluate, review-record and promote stages of F23-01; marks no requirement complete.</action>
  <verify>
    ...
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; L=crates/wcore-skills/src/loader.rs; test "$(/usr/bin/grep -ciF 'governance' "$L")" -ge 1 || { echo "the loader gained no governance-aware entry point, so a promotion taken at the CLI stays invisible to the next session's catalog"; exit 1; }; test "$(/usr/bin/grep -cF 'is_generated_draft' "$L")" -ge 1 || { echo "REGRESSION GUARD (not coverage): the provenance classifier the default-quarantine behaviour keys on is gone"; exit 1; }</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; B=$(cat .planning/phases/23A-governed-skills/23A-02-BASE-SHA); test -n "$B" || { echo "base SHA not recorded"; exit 1; }; N=$( { /usr/bin/git diff -- Cargo.lock; /usr/bin/git diff "$B"..HEAD -- Cargo.lock; } | /usr/bin/grep -cE '^[-+]version = '); test "$N" -eq 0 || { echo "Cargo.lock moved $N package version lines"; exit 1; }</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; B=$(cat .planning/phases/23A-governed-skills/23A-02-BASE-SHA); test -n "$B" || { echo "base SHA not recorded"; exit 1; }; OUT=$(/usr/bin/git status --porcelain -- crates/ ':(exclude)crates/wcore-memory/src/schema/*' ':(exclude)crates/wcore-skills/src/governance.rs' ':(exclude)crates/wcore-skills/src/lib.rs' ':(exclude)crates/wcore-skills/src/loader.rs' ':(exclude)crates/wcore-skills/Cargo.toml' ':(exclude)crates/wcore-skills/tests/governed_promotion.rs' ':(exclude)crates/wcore-agent/src/bootstrap.rs'; /usr/bin/git diff --name-only "$B"..HEAD -- crates/ ':(exclude)crates/wcore-memory/src/schema/*' ':(exclude)crates/wcore-skills/src/governance.rs' ':(exclude)crates/wcore-skills/src/lib.rs' ':(exclude)crates/wcore-skills/src/loader.rs' ':(exclude)crates/wcore-skills/Cargo.toml' ':(exclude)crates/wcore-skills/tests/governed_promotion.rs' ':(exclude)crates/wcore-agent/src/bootstrap.rs'); test -z "$OUT" || { printf 'this task touched crate files outside its declared sources:\n%s\n' "$OUT"; exit 1; }</automated>
...
Implements the operator half of the evaluate, review and promote stages; marks no requirement complete.</action>
  <verify>
    ...
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; M=crates/wcore-cli/src/main.rs; test "$(/usr/bin/grep -cF 'skills_cmd' "$M")" -ge 1 || { echo "the governed subcommand is not wired into the top-level subcommand list"; exit 1; }; test "$(/usr/bin/grep -cF 'skills_archive' "$M")" -ge 1 || { echo "REGRESSION GUARD (not coverage): the pre-existing archive dispatch was removed rather than left unchanged"; exit 1; }</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; B=$(cat .planning/phases/23A-governed-skills/23A-02-BASE-SHA); test -n "$B" || { echo "base SHA not recorded"; exit 1; }; OUT=$(/usr/bin/git status --porcelain -- crates/wcore-cli/ ':(exclude)crates/wcore-cli/src/skills_cmd.rs' ':(exclude)crates/wcore-cli/src/lib.rs' ':(exclude)crates/wcore-cli/src/main.rs' ':(exclude)crates/wcore-cli/tests/skills_lifecycle_cmd.rs'; /usr/bin/git diff --name-only "$B"..HEAD -- crates/wcore-cli/ ':(exclude)crates/wcore-cli/src/skills_cmd.rs' ':(exclude)crates/wcore-cli/src/lib.rs' ':(exclude)crates/wcore-cli/src/main.rs' ':(exclude)crates/wcore-cli/tests/skills_lifecycle_cmd.rs'); test -z "$OUT" || { printf 'this task touched wcore-cli files outside its four declared sources:\n%s\n' "$OUT"; exit 1; }</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; B=$(cat .planning/phases/23A-governed-skills/23A-02-BASE-SHA); test -n "$B" || { echo "base SHA not recorded"; exit 1; }; F=crates/wcore-cli/tests/skills_lifecycle_cmd.rs; test "$(/usr/bin/grep -cF 'async fn skills_archive_transitions_staged_to_archived' "$F")" -eq 1 || { echo "the fourth lifecycle case is missing or duplicated"; exit 1; }; H1=$(/usr/bin/git show "$B:$F" | /usr/bin/awk '/^async fn skills_archive_transitions_staged_to_archived/,/^}/' | /usr/bin/shasum | /usr/bin/cut -d' ' -f1); H2=$(/usr/bin/awk '/^async fn skills_archive_transitions_staged_to_archived/,/^}/' "$F" | /usr/bin/shasum | /usr/bin/cut -d' ' -f1); test "$H1" = "$H2" || { echo "the fourth lifecycle case skills_archive_transitions_staged_to_archived was altered; it is explicitly not to be weakened"; exit 1; }</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox; B=$(cat .planning/phases/23A-governed-skills/23A-02-BASE-SHA); test -n "$B" || { echo "base SHA not recorded"; exit 1; }; F=crates/wcore-cli/tests/skills_lifecycle_cmd.rs; NEW=$(/usr/bin/grep -cE '^[[:space:]]*#\[(tokio::)?test' "$F"); OLD=$(/usr/bin/git show "$B:$F" | /usr/bin/grep -cE '^[[:space:]]*#\[(tokio::)?test'); test "$NEW" -gt "$OLD" || { echo "the lifecycle suite carries $NEW cases against the $OLD it started with; the three suspension cases were to be replaced by strictly stronger governed ones, so the measured count must rise"; exit 1; }; test "$(/usr/bin/grep -cF '#[ignore]' "$F")" -eq 0 || { echo "a case was ignored rather than replaced"; exit 1; }</automated>
    ...
    <automated>SHA=$(cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; /usr/bin/git rev-parse HEAD); ssh -o BatchMode=yes SeanD@seandesktop "cmd /c \"cd /d C:\ferrox-win &amp;&amp; git fetch origin plan/f20-unified-audit-repair &amp;&amp; git checkout --detach $SHA &amp;&amp; git rev-parse HEAD &amp;&amp; cargo clippy --workspace --all-targets -- -D warnings &amp;&amp; cargo nextest run --profile ci -p wcore-cli -p wcore-skills --no-fail-fast\"; exit \$LASTEXITCODE" &gt; /tmp/f23a-02-cli-win.log 2&gt;&amp;1; rc=$?; /usr/bin/grep -v CLIXML /tmp/f23a-02-cli-win.log &gt; /tmp/f23a-02-cli-win.clean 2&gt;/dev/null; tail -60 /tmp/f23a-02-cli-win.clean; test $rc -eq 0 || { echo "the Windows clippy-then-test leg exited $rc; the CLIXML-filtered transcript is above and the raw capture is /tmp/f23a-02-cli-win.log"; exit $rc; }</automated>
  </verify>
...
<task type="auto">
  ...
  <files>crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs, scripts/f23a-promotion-drive.sh, scripts/f23a-promotion-drive.ps1, .planning/phases/23A-governed-skills/23A-02-LIVE-EVIDENCE.md</files>
  <read_first>crates/wcore-eval-scenarios/src/governed_skill_drive.rs (23A-01's shared drive harness — the tempdir home-and-project seeding that enables the skills lifecycle, the helper that drives the product's own drafting path until a real `auto-*` draft exists, the helper that invokes an operator verb as a separate `wayland-core` process with argv, and the two self-test switches; this task REUSES it and does not fork it), crates/wcore-eval-scenarios/tests/f23a_boundary_drive.rs (23A-01's drive target: the fixture-step shape, the assertion style that reads the captured transcript and trace rather than an internal flag, and the positive control), crates/wcore-eval-scenarios/src/fixtures/openai.rs (`OpenAiFixtureScript` at 34, `OpenAiStep` at 41, `tool_call` at 105, `base_url` at 407 — the loopback provider), crates/wcore-eval-scenarios/src/runner.rs (`run_with_binary_in_paths` at 462 and the argv it builds at 338-346), scripts/f23a-boundary-drive.sh (23A-01's thin wrapper: the expected-SHA assertion with its pinned exit 3, the build-and-pin sequence, and the fact that it carries no assertion logic of its own — this wrapper mirrors it exactly so the two are comparable), .planning/phases/23A-governed-skills/23A-01-LIVE-EVIDENCE.md (the exact recorded refusal this plan must invert for the same draft — the before half of the before-and-after already exists and is not re-derived), crates/wcore-cli/src/skills_cmd.rs (the verbs, their stable stdout tokens and their exit codes, so the drive target observes outcomes rather than parsing prose), justfile (the f01-packaged-driver-gate recipes at 165-181 for unix and 183-184 for windows: `WAYLAND_BUILD_SOURCE_SHA` from `git rev-parse HEAD`, the absolute `WCORE_EVAL_BIN` derivation with the `.exe` suffix on Windows, and the `exit $LASTEXITCODE` discipline)</read_first>
  <behavior>
    - The live mechanism is 23A-01's, inherited: the drive target is `crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs` built on `wcore_eval_scenarios::governed_skill_drive`, and the two shell files are thin wrappers whose exit status is `cargo test -p wcore-eval-scenarios --features packaged-driver-gate --test f23a_promotion_drive`. No harness is forked, no draft is hand-seeded, and this leg is never degraded to the existing packaged gate.
    ...
    - The wrapper asserts its own checkout SHA before acting and exits EXACTLY 3 on mismatch.
    - `WAYLAND_F23A_SELFTEST=unreviewed` makes the drive target promote the sibling without recording a review and then assert it executes, print `F23A-SELFTEST-TRIPPED: unreviewed`, and exit nonzero — proving the unreviewed control actually fires rather than merely appearing in a comment.
  ...
  <action>Build this the way 23A-01 established, and inherit rather than reinvent. The drive target is `crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs`, feature-gated on `packaged-driver-gate`, built on the shared `governed_skill_drive` module: an `OpenAiFixtureScript` loopback provider, the real binary spawned through `--json-stream` by `run_with_binary_in_paths`, a draft produced by the PRODUCT's own drafting path, and the review and promote verbs invoked as separate `wayland-core` processes with argv the way `packaged_driver_gate.rs:267` does. The `.sh` and `.ps1` files are thin wrappers and nothing more: assert `WAYLAND_EXPECT_SHA` against `git rev-parse HEAD` and exit EXACTLY 3 on mismatch; export `WAYLAND_BUILD_SOURCE_SHA`; `cargo build --locked -p wcore-cli --bin wayland-core`; derive an absolute `WCORE_EVAL_BIN` (with `.exe` on Windows) exactly as `justfile:165-181` and `justfile:183-184` do; then `cargo test --locked -p wcore-eval-scenarios --features packaged-driver-gate --test f23a_promotion_drive` and exit with ITS status. Assertion logic in a wrapper is logic nobody tests, so there is none.
...
Exit nonzero on any deviation, and specifically on either control succeeding. A driver where every step is expected to pass and nothing is expected to fail cannot go red for the reason that matters — and a control nobody ever proved fires is the same thing with extra words, which is why `WAYLAND_F23A_SELFTEST=unreviewed` must promote the sibling without a review, assert it executes, print `F23A-SELFTEST-TRIPPED: unreviewed`, and fail. Implement both self-tests in the drive target so Linux and Windows inherit one implementation.
...
Closes the evaluate, review and promote stages of F23-01 at the product surface for Linux and Windows; marks no requirement complete — closure is claimed by 23A-04.</action>
  <verify>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; cargo fmt --all -- --check</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; test -x scripts/f23a-promotion-drive.sh &amp;&amp; test -f scripts/f23a-promotion-drive.ps1 &amp;&amp; test -f crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs &amp;&amp; bash -n scripts/f23a-promotion-drive.sh</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; test "$(/usr/bin/grep -cF 'f23a_promotion_drive' scripts/f23a-promotion-drive.sh)" -ge 1 &amp;&amp; test "$(/usr/bin/grep -cF 'f23a_promotion_drive' scripts/f23a-promotion-drive.ps1)" -ge 1 &amp;&amp; test "$(/usr/bin/grep -cF 'governed_skill_drive' crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs)" -ge 1</automated>
    ...
    <automated>SHA=$(cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; /usr/bin/git rev-parse HEAD); ssh -o BatchMode=yes hetzner-dsm "cd /root/wayland &amp;&amp; WAYLAND_EXPECT_SHA=0000000000000000000000000000000000000000 bash scripts/f23a-promotion-drive.sh; echo SHA_RC=\$?; WAYLAND_EXPECT_SHA=$SHA WAYLAND_F23A_SELFTEST=unreviewed bash scripts/f23a-promotion-drive.sh; echo SELFTEST_RC=\$?" &gt; /tmp/f23a-02-selftest-linux.log 2&gt;&amp;1; /usr/bin/grep -qF 'SHA_RC=3' /tmp/f23a-02-selftest-linux.log &amp;&amp; /usr/bin/grep -qE 'SELFTEST_RC=[1-9]' /tmp/f23a-02-selftest-linux.log &amp;&amp; /usr/bin/grep -qF 'F23A-SELFTEST-TRIPPED: unreviewed' /tmp/f23a-02-selftest-linux.log || { echo "promotion driver self-tests did not fire — the unreviewed control is decorative"; tail -40 /tmp/f23a-02-selftest-linux.log; exit 1; }</automated>
    <automated>SHA=$(cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; /usr/bin/git rev-parse HEAD); ssh -o BatchMode=yes SeanD@seandesktop "cmd /c \"cd /d C:\ferrox-win &amp;&amp; git fetch origin plan/f20-unified-audit-repair &amp;&amp; git checkout --detach $SHA\"; \$env:WAYLAND_EXPECT_SHA='$SHA'; powershell -NoProfile -File C:\ferrox-win\scripts\f23a-promotion-drive.ps1; exit \$LASTEXITCODE" &gt; /tmp/f23a-02-drive-win.log 2&gt;&amp;1; rc=$?; /usr/bin/grep -v CLIXML /tmp/f23a-02-drive-win.log &gt; /tmp/f23a-02-drive-win.clean 2&gt;/dev/null; tail -80 /tmp/f23a-02-drive-win.clean; test $rc -eq 0 || { echo "the Windows promotion driver exited $rc; the CLIXML-filtered transcript is above and the raw capture is /tmp/f23a-02-drive-win.log"; exit $rc; }</automated>
    <automated>SHA=$(cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; /usr/bin/git rev-parse HEAD); ssh -o BatchMode=yes SeanD@seandesktop "\$env:WAYLAND_EXPECT_SHA='0000000000000000000000000000000000000000'; powershell -NoProfile -File C:\ferrox-win\scripts\f23a-promotion-drive.ps1; Write-Output \"SHA_RC=\$LASTEXITCODE\"; \$env:WAYLAND_EXPECT_SHA='$SHA'; \$env:WAYLAND_F23A_SELFTEST='unreviewed'; powershell -NoProfile -File C:\ferrox-win\scripts\f23a-promotion-drive.ps1; Write-Output \"SELFTEST_RC=\$LASTEXITCODE\"" &gt; /tmp/f23a-02-selftest-win.log 2&gt;&amp;1; /usr/bin/grep -qF 'SHA_RC=3' /tmp/f23a-02-selftest-win.log &amp;&amp; /usr/bin/grep -qE 'SELFTEST_RC=[1-9]' /tmp/f23a-02-selftest-win.log &amp;&amp; /usr/bin/grep -qF 'F23A-SELFTEST-TRIPPED: unreviewed' /tmp/f23a-02-selftest-win.log || { echo "Windows promotion driver self-tests did not fire"; /usr/bin/grep -v CLIXML /tmp/f23a-02-selftest-win.log | tail -40; exit 1; }</automated>
  ...
  <done>The drive target `crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs` exists, reuses 23A-01's `governed_skill_drive` harness rather than forking it, and both shell files are thin wrappers that name it and carry no assertion logic. One continuous run per platform drives draft, refusal, review, promotion, execution, and both controls. The unreviewed sibling is still refused and the post-edit body is refused again, and the driver exits nonzero if either control succeeds. Both wrappers exit exactly 3 on an expected-SHA mismatch, and `WAYLAND_F23A_SELFTEST=unreviewed` makes both fail with the tripped marker, so the unreviewed control is proved to fire. 23A-01's boundary driver still passes at this SHA, so promotion did not widen the quarantine boundary it was built on top of. `23A-02-LIVE-EVIDENCE.md` records every invocation, its output, its exit code, the before-and-after side by side, both controls, both self-test outcomes and the bound artifact hash, and states that macOS is 23A-04's disposition. No gate took its status from a pipeline, and no untouched-surface claim rests on a bare `git status --porcelain` or `git diff --stat`.</done>
</task>
...
<verification>
Local gates (Mac, source level only — Cargo is never run here): `cargo fmt --all -- --check` clean; the sixth migration and the kernel module exist; the version constant reads 6; the kernel is exported and the hashing dependency is declared; the new subcommand module exists and is both exported and wired; the archive flag still exists; the lock moves no package version measured across BOTH the working tree and the base-SHA range, so a committed lock bump cannot hide; nothing under `crates/` or `crates/wcore-cli/` was touched outside each task's declared files, measured the same two ways; the fourth lifecycle case `skills_archive_transitions_staged_to_archived` hashes byte-identical to its base-SHA body and no `#[ignore]` appears in that file; the drive target exists, names the shared harness, and both wrappers name the drive target.
...
- The live mechanism is 23A-01's `governed_skill_drive` harness, inherited not forked: an `OpenAiFixtureScript` provider, the real binary through `--json-stream`, the product's own drafting path, and operator verbs as separate processes; the `.sh` and `.ps1` files are thin wrappers around `cargo test ... --test f23a_promotion_drive` and carry no assertion logic.
- Both wrappers exit exactly 3 on an expected-SHA mismatch, and `WAYLAND_F23A_SELFTEST=unreviewed` makes both fail with a tripped marker — so the unreviewed control is proved to fire rather than merely counted by a grep.
- No gate derives its status from a pipeline, from an exit code read out of an output-emitting block, or from a bare `git status --porcelain`, `git diff --stat` or `git diff --name-only`; every untouched-surface and lock-drift claim is `test -z` or an explicit count over both a working-tree leg and a base-SHA leg.
...
- `.planning/phases/23A-governed-skills/23A-02-BASE-SHA` — the untouched starting commit every untouched-surface and lock-drift gate measures against.
- `crates/wcore-eval-scenarios/tests/f23a_promotion_drive.rs` — the Rust drive target, built on 23A-01's shared harness, whose exit status is the live before-and-after proof.
- `scripts/f23a-promotion-drive.sh` and `scripts/f23a-promotion-drive.ps1` — the SHA-asserting thin wrappers around that drive target.
