...
Errors are thiserror-structured and name the offending path. Addresses F23-02; marks no requirement complete.</action>
  <verify>
    ...
    <!-- COMPLETION gate. The bare count that stood here passed on the untouched tree: the module does not exist, grep matched nothing, and the count was 0. The leading existence and declaration clauses are what make this RED at base; the trailing count is a quality clause ON THE FILE THIS TASK CREATES, not standalone coverage. -->
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; test -f crates/wcore-agent/src/session_lifecycle.rs &amp;&amp; /usr/bin/grep -qE '^ *(pub )?mod session_lifecycle;' crates/wcore-agent/src/lib.rs &amp;&amp; test "$(/usr/bin/grep -v '^ *//' crates/wcore-agent/src/session_lifecycle.rs | /usr/bin/grep -cF 'panic!')" -eq 0</automated>
  </verify>
...
Addresses F23-02; marks no requirement complete.</action>
  <verify>
    ...
    <!-- COMPLETION gate. Same defect as Task 1's: the bare count passed on the untouched tree because the module is absent. The existence and re-export clauses make it RED at base; the trailing count is a quality clause on the created file, not coverage. -->
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; test -f crates/wcore-cli/src/session_cmd.rs &amp;&amp; test -f crates/wcore-cli/tests/session_operator_lifecycle.rs &amp;&amp; /usr/bin/grep -qE '^ *(pub )?mod session_cmd;' crates/wcore-cli/src/lib.rs &amp;&amp; test "$(/usr/bin/grep -v '^ *//' crates/wcore-cli/src/session_cmd.rs | /usr/bin/grep -cEi 'todo!|unimplemented!')" -eq 0</automated>
  </verify>
...
Write `23B-01-LIVE-EVIDENCE.md` as a table: one row per verb per platform, carrying the exact invocation, the observed stdout token, the exit code, the on-disk consequence and a PASS, RED or OPEN verdict. A verb that did not run is OPEN with its reason, never silently absent. Then state F23-02's disposition: complete only if every verb is PASS on all three platforms, otherwise incomplete with the open rows named. Marks F23-02 complete only under that condition; otherwise records the disposition and leaves it incomplete.</action>
  <verify>
    ...
    <!-- Verb coverage is asserted against the CAPTURED DRIVE LOGS written by the three gates above, never against 23B-01-LIVE-EVIDENCE.md. Grepping that table proved only that the executor typed into it. These two gates require the driver to have actually emitted, per platform, its nonce-bound terminal marker and one machine-readable outcome line per verb — the marker vocabulary the driver already owes. RED at base: no log exists. -->
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; for P in linux macos windows; do L=.planning/phases/23B-continuous-agency/evidence/23B-01-$P-drive.log; test -f "$L" || exit 1; N=$(/usr/bin/grep -oE 'nonce=[0-9a-f]{16}' "$L" | tail -1 | cut -d= -f2); test -n "$N" || exit 1; /usr/bin/grep -qF "F23_01_DRIVE=PASS platform=$P nonce=$N" "$L" || exit 1; for V in list search show checkpoint retry fork rewind export retain reconcile; do /usr/bin/grep -qE "F23_01_VERB=$V platform=$P status=PASS exit=[0-9]+ nonce=$N" "$L" || exit 1; done; done</automated>
    <automated>cd /Users/seandonahoe/dev/waylandcore-ferrox &amp;&amp; for P in linux macos windows; do L=.planning/phases/23B-continuous-agency/evidence/23B-01-$P-drive.log; /usr/bin/grep -qE '^F23_01_EXPORT_NONCE_OCCURRENCES=0$' "$L" &amp;&amp; /usr/bin/grep -qE '^F23_01_REWIND_BYTES_EQUAL=true$' "$L" &amp;&amp; /usr/bin/grep -qE '^F23_01_FORK_PARENT_BYTES_EQUAL=true$' "$L" &amp;&amp; /usr/bin/grep -qE '^F23_01_RECONCILE_RESOLVED_PERSISTS_ACROSS_RESTART=true$' "$L" || exit 1; done</automated>
  </verify>
