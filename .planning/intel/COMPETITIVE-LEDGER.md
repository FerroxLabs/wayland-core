...
| REACH-* | backends, nodes, plugins | shared | core | **REACHED** *(was SOURCE)* | `PEER-PROBE-2026-07-26`; `GAP-AUDIT-2026-07-13 §3`; `F25-01-BACKENDS`; `F25-02-PLUGINS`; `F25-03-NODES`; `F25-04-FAILCLOSED` | `BASE-2026-07-13` — Hermes 0.17.0 @ `dbe734be`; OpenClaw 2026.6.2 @ `11a0ad10` | Remote execution: **Core behind Hermes**; extension distribution: **architecture competitive, ecosystem behind** (`GAP-AUDIT §3`). Probe: Hermes ships **seven** execution backends behind one `base.py` contract — `tools/environments/{local,docker,ssh,singularity,modal,managed_modal,daytona}.py` plus `file_sync.py`. OpenClaw ships `src/node-host`, `src/plugins`, `src/plugin-sdk`, `src/plugin-state`, `packages/plugin-sdk`, `packages/plugin-package-contract`, `extensions/`. Core has `wcore-plugin-api` and sandbox/worktree assurance but **no user-facing execution-backend matrix** | **MOVES TO REACHED — the strongest Core-side result in this refresh, and the only family carrying a MET Success Criterion.** The previous limitation said plainly "the F25-01 provider-neutral contract does not exist"; it exists, and three of its four reference backends **ran the same deterministic task through the shipped binary and diffed to EQUIVALENT** (`F25-01-BACKENDS`). All twelve `wayland-core plugin` verbs plus all four negative cases were driven through the shipped **release** binary on **Linux and Windows**, with approval enforced at load time and bound to the plugin directory's SHA-256 so an update invalidates consent — **Success Criterion 3 MET on Linux, MET-with-one-divergence on Windows** (`F25-02-PLUGINS`). Node identity is attested inside the signed receipt and survived all five disruptions (`F25-03-NODES`); all five hostile compromise cases fail closed on both hosts (`F25-04-FAILCLOSED`). **The deciding difference from GOAL-\* and GATEWAY-\*, stated because those calls are close:** this family carries **two-platform live product exercise on the criterion that passed** and — unlike GOAL-* — has **no mapped F05 identity recorded `runtime path unwired`**. ~~**Not EFFECTIVE:** three of four criteria are NOT MET. C1 fails on the hibernating cloud leg, unexercised for want of a credential only Sean can mint. C2 fails because everything was exercised against a separate machine *identity* but **not a second physical host** — no SSH trust relationship exists between `hetzner-dsm` and `SeanD@seandesktop` and creating one is reserved to Sean. C4 fails because SSH and cloud cannot be enumerated on the proof hosts, so their orphan counts are `NOT MEASURED`~~ — **CORRECTED 2026-07-30 by `lane/reach-c1-c2`. All three "reserved to Sean" clauses were false, and two of them were already known false when this cell was written.** (1) **C2's SSH trust already existed**: `ssh hetzner-dsm 'ssh SeanD@seandesktop hostname'` → `SeanDesktop`, rc 0, re-measured 2026-07-30 with two known-negative controls that both return 255 (unresolvable host; wrong account). `lane/25-hosts` measured the identical result on **2026-07-28** and ran the whole node corpus across the two physical hosts — the cell was authored citing `Phase 25, 2026-07-28` and never refreshed. (2) **C1's credential was minted by Sean on 2026-07-28 and is still live**: `/root/.wayland-f25-cloud.env`, `0600`, and the product's own probe reads back `available: true / probe basis: VendorApiCall / vendor API answered 200 for app wayland-f25-test` at HEAD, with a credential-absent control arm proving the probe can say no. The backend is **Fly.io Machines**, not GCP/Azure. (3) **C4's SSH and cloud orphan surfaces are both MEASURED**, each in both directions. **All of it independently re-verified at integration head `0d48b551`** — no prior lane had re-measured any of this above its own tip: four surfaces (local/container/ssh/cloud) run the same task in **one** invocation and diff `NORMALIZED DIFF: EQUIVALENT`, with the gate proven able to report `DIVERGENT` (exit 1) on a receipt that is itself `INTEGRITY: OK`; ssh orphan `found=2` on an orphan **the product leaked** (controller `kill -9`, participants asserted present first) against `found=0` for an unused nonce at the same instant, then `found=0` after a real `backend cancel` corroborated by an independent far-end `ps` of 0; cloud orphan `found=1` against a raw vendor-API census of 1 taken **outside the product**, `found=0` for an unused nonce, and `0` on both instruments after teardown — no billable machine leaked. Secret sweep over 69 files: **0** token hits, known-positive 4, known-negative 0. **The remaining blockers are real but none is Sean's**: the verdict `25-PHASE-VERDICT.md` (2026-07-29) graded C1 and C4 **PARTIAL** and costed gaps **G1–G12 — every one marked "Credential? NO"**; G1/G2/G3 were then closed by `lane/25-c1-cleanup`, G4/G5/G6 by `lane/25-c4-egress` and `lane/25-c4-windows`, and **all of those fixes are present at `0d48b551`** (verified by source markers with known-positive and known-negative controls). What is genuinely still open is build work: the un-denied cloud **POST** egress path on both platforms, the missing `--i-accept-exfil-risk` two-key interlock (an **owner** decision, not a credential), G7 controller-side receipt identity, and `setsid` on a Windows ssh far end. **Open question for F30 — not resolved here:** the Delta's "Core has … **no user-facing execution-backend matrix**" is a statement about Core that `wayland-core backend` falsified; the seven-backend Hermes comparison is untouched | Phase 25 (25-01…25-04); re-verified at `0d48b551` by `lane/reach-c1-c2`, 2026-07-30 | Nothing reserved to Sean. Build work only: cloud POST egress denial; `--i-accept-exfil-risk` (owner decision); G7 controller-side attribution; `setsid` on a Windows ssh far end. C2's node corpus and the Windows legs were **not** re-run at `0d48b551` by this lane |
...
second-host SSH trust relationship;~~ **CORRECTED 2026-07-30 by `lane/reach-c1-c2`: both REACH-\*
items were false. C2's `hetzner-dsm` → `SeanD@seandesktop` trust already existed and had already
been measured on 2026-07-28; C1's Fly credential was minted on 2026-07-28, is still on the proof
host, and answers HTTP 200 at HEAD. Neither was ever re-checked before being written down here.**
The remaining item is GATEWAY-*'s macOS leg, which needs a CI-trigger change that is a PR
action. It does not block the ledger's schema; it blocks a specific promotion.

**The recurring failure this correction makes concrete:** a "blocked on Sean" note is an
**absence claim about the world** and is subject to the same rule as every other absence in this
programme — it must be re-measured before it is repeated. Three such notes have now been written
into this file and later found false (`25-PHASE-STATUS.md`'s SSH-trust blocker, and both REACH-*
clauses above). Each cost a reserved round trip that was never needed. Before recording that
something waits on Sean, run the one command that would falsify it.
...
| REACH-* | SOURCE | **REACHED** | **Promoted.** Two-platform live product exercise and the only MET Success Criterion in phases 21-29. ~~Three of four criteria still NOT MET.~~ **Corrected 2026-07-30 by `lane/reach-c1-c2`:** the three criteria were graded PARTIAL / MET-WITH-EXCEPTIONS by `25-PHASE-VERDICT.md`, not NOT MET, and **none of the row's three "reserved to Sean" blockers was real** — the second-host SSH trust already existed, the Fly credential was already minted and still answers 200, and the SSH and cloud orphan surfaces are both MEASURED in both directions. Independently re-verified at integration head `0d48b551`: four surfaces EQUIVALENT in one invocation with a gate proven able to say DIVERGENT; ssh and cloud orphans found-then-reaped against independent instruments; 0 billable leak. What remains is build work, not a credential. |
