...
| `22-C1` | **PARTIAL** | unchanged | Typed Goal *control* landed on all three surfaces (5 command variants at `commands.rs:328-340` + a typed refusal event); the producer-fixture clause is closed. Re-verified at HEAD: `TuiEngine::issue_goal_control` **still has ZERO call sites** (definition at `engine_bridge.rs:1217` plus two comments), against a known-positive caller-search of **12 refs across 7 files** for `run_stop_hooks`. No TUI user can reach Goal control. |
| `22-C3` | **PARTIAL** | unchanged | Half A advanced — the last representable engine-verdict bypass is shut at the durable boundary for 5/5 owners and any sixth; un-goaled invocation stays opt-in; `GoalKernel::terminate` still `pub` (`kernel.rs:146`). Half B closed, pre-existing. **Its falsifier is still a dead instrument** — see the section below. |
| `22-C4` | **PARTIAL** | unchanged | One measurement moved; the gap did not. `start_iteration` has **2** production callers at HEAD (`goal/control.rs:431`, `goal/fleet.rs:475`); neither is one of the four non-Fleet loop owners the criterion names. |
...
| `24-C1` | **PARTIAL** | **unchanged grade — JUSTIFICATION WAS FALSE, see below** | The platform half is closed; the conjunction *"no delivery lost **and** none duplicated"* is not. **Exactly-once is 1 of 10 — Matrix alone.** This row previously said *"3 of 10 — Slack, Matrix, Discord"* and that was a **false customer-facing guarantee**; Slack and Discord were each driven at their real API on 2026-07-30 and each produced **two** messages from a replayed key, both having held the claim on mockito evidence. No-loss fails on **9** of 10. Seven platforms provide no idempotency primitive at all. **DECIDED 2026-07-31 (Sean) — see docs/decisions/0005: keep at-most-once, no auto-retry, and RE-SCOPE this criterion.** As written, *"no delivery lost AND none duplicated"* is unsatisfiable on those seven for reasons outside the codebase, so the row had **no reachable pass state** — the permanently-red-gate failure this project treats as equal to a gate that cannot fail. Re-scoped to: *no delivery is lost **silently**; every outcome-unknown delivery is recorded and recoverable by an operator* — already true and shipping (`wayland-core gateway abandoned`, #109). **Residual is measurement, not design:** the at-most-once claim is still **NOT MEASURED at a real destination on 7 of 10** adapters, which is exactly the state Slack and Discord were in the morning they were falsified. |
...
| `24-C1` | replay at a real destination for 7 of 10 adapters | Telegram/Twilio/Meta/SMTP/Signal/iMessage/Teams credentials we do not hold. The product decision is **MADE** (docs/decisions/0005, 2026-07-31): at-most-once stays, criterion re-scoped. What is left is driving the recovery queue live wherever we DO hold credentials. |
