...
**Plans**: Eighteen dependency-ordered stock-GSD plans (`20-01` through `20-18`). Foundational isolation source plan `20-03` is independently reviewed by review-only plan `20-15` before caller wiring begins in `20-04`. The original candidate-gate plan is split into 06A source/review (`20-06`, `20-09`), 06B source/review (`20-10`, `20-11`), 06C private authoritative execution (`20-12`), 06D black-box proof (`20-13`), and an exact-candidate independent integrated audit (`20-14`). Caller integration (`20-05`) resumes only after `20-14`; parent CAS (`20-07`) follows `20-05`. Plan `20-08` constructs and focused-proves the final lifecycle/UAT machinery; review-only `20-16` prosecutes that exact source at every severity; autonomous plan `20-17` durably prepares one idempotent pending request without external mutation; terminal plan `20-18` alone owns requirements and runs the explicitly authorized candidate-specific native Windows/macOS plus aggregate Hetzner acceptance. The later stock-GSD phase verifier authenticates that exact-candidate proof.

The 20-18 native UAT ran RED against the pre-repair candidate; ten **additive** native-repair successor plans (`20-19` through `20-28`) close the native path without replanning or re-executing accepted `20-01…20-16`. They satisfy REQ-native-r1…r15: `20-19` hard-resets the tainted tree to pristine `be84bd2` and writes the real sandbox source fixes fresh (is_available, drop deny-only SIDs with isolation preserved, Windows import); `20-20` fixes the two Windows test bugs and authors the AppContainer isolation proofs (genuine DENY blocks, normal-SID-only denied); `20-21` authors the real Windows Job-Object containment tests, repoints the two mis-wired harness targets, and adds the wrong-OS anti-drift guard; `20-22` re-validates the macOS harness, pins the Windows leg to the self-hosted msvc runner, closes the UAT-proof writer gap, and adds the native-inclusive review profile; `20-23` seals ONE exact repaired candidate (regenerated `Cargo.lock`, Hetzner build + aggregate 11509/0 no-regression); `20-24` is the pre-native fresh cross-audit (schema-validated artifact per reviewer); `20-25` is the Sean-gated native-proof dispatch (Windows self-hosted msvc + macOS ephemeral, re-proving the actual 07-22 compile/mapping failures on hardware); `20-26` is the fresh independent review with native NOT deferred (schema-validated per reviewer); `20-27` re-prepares one idempotent pending terminal tuple; and terminal `20-28` takes Sean's exact-tuple authorization, confirms retained dual-OS native green + aggregate Hetzner, and completes the Phase 20 requirements.
