...
criteria:
  ...
  - id: c3
    text: "The retry-masked rate is on the record for both arms, split by ancestry rather than by date, with the executions counted and the instrument stated. Where a --retries 0 arm at a named load is not taken, the record says so and states what the recorded incidence is NOT."
    state: met
    evidence: "file:.planning/FLAKE-CENSUS-20260910.md"
    owner: core
    note: "Transcribed from the issue body verbatim on 2026-08-31. This ledger did not exist until now: the issue was filed 2026-08-29/30 by this cycle's own verification and never entered the release gate, which counts only issues holding a ledger file. State is not-met because no lane has claimed it and nothing in the tree has been graded against this text. kind is defect, not task, because the gate reserves task for a credential, an account or a platform a human must obtain and there is code behind this one. RESTATED ON THE ISSUE 2026-09-10 and graded against the restatement: https://github.com/FerroxLabs/wayland/issues/1238#issuecomment-5617877997. WHY THE ORIGINAL COULD NOT BE MET BY THIS RELEASE: it demanded n>=20 at --retries 0 AT LOADAVG ABOVE 190 with a deliberately-broken-cap known-positive control in the same run and both binaries identified by sha256 -- an executed measurement on an enforcing host that no lane took a slot for. WHAT IS ON THE RECORD INSTEAD, split BY ANCESTRY rather than by date (compare/774c40f5a...<headSha>; 59 runs carry the fix, 65 do not): BEFORE 183 executions with 2 `TRY n FAIL` (run 33719833206, CI (Array)) plus 1 hard failure (33584616526, windows-hosted); AFTER 170 executions with 0 `TRY n FAIL` and 0 hard failures. THIS DOES NOT DEMONSTRATE THE FIX WORKED and is not offered as if it did: 1/183 against 0/170 is not distinguishable at these n -- the rule of three puts the AFTER bound near 1.8%, which contains 0.55%. The figures are per-execution incidence under retries = 2, a DIFFERENT QUANTITY from a --retries 0 rate, measured under different load, and that distinction is the whole reason c3's own condition still stands unreached."
  - id: c4
