...
criteria:
  ...
  - id: c6
    ...
    state: not-met
    evidence: "symbol:crates/wcore-agent/src/engine.rs::attach_transient_block"
    owner: core
    note: "RE-OWNED 2026-08-31 during the f13 landing pass: was blocked / owner: maintainer with no handoff, which the release gate correctly refuses -- a blocked criterion naming no carrier is one that has stopped being anyone job. There is no maintainer decision left to take here, so it goes back to core as ordinary work with the work stated. WHY THE BLOCK WAS WRONG: c6 is a POSITIONAL sentence, and the harm it stands for -- turn 1 becoming a cache entry no later turn can read, which is what produced the 77.7M input tokens at cache_read=0 -- is graded DIRECTLY by c7, whose evidence the_first_dispatch_writes_no_cache_entry_at_a_transient_head is met in this tree. So the question c6 was blocked on is answered. NOT GRADED MET ON c7 EVIDENCE, deliberately: discharging a criterion with a neighbouring property is the most common false close on this release, and c6 sentence is still false at HEAD. WHAT IS ACTUALLY OWED, so the next lane does not re-derive it: engine.rs:8291 attach_transient_block inserts the skill-router hint and the PrePrompt contributions into the content of whatever message the caller hands it, and the caller passes request.messages.last_mut(), which on turn 1 IS messages[1]. Closing c6 as written means attaching a turn-1 transient somewhere that is not the turn-1 boundary. THE CONSTRAINT ON ANY SUCH FIX, and the reason this is not a one-line move: c7 must stay green, so whatever position is chosen must still not create a cache WRITE point at the head of turn 1 -- the two criteria are coupled and a fix graded against only one of them is worth nothing."
  - id: c7
