...
criteria:
  ...
  - id: c5
    ...
    state: not-met
    owner: core
    note: "BLOCKED ON AN UNMERGED LANE, not on this defect. Handed over intact from core#361 c6. The three other shared-process races that stand in the way of a clean run are fixed on `lane/f13-fin-flake-584` (`2c8efb46` wcore-config, `812e26075` wcore-observability, `18e59e85f` wcore-cli) and NONE of them is in `origin/integ/f13`, so the ten consecutive clean runs are unreachable from this lane's tree no matter what happens to the osv defect. Measured here rather than assumed -- see the prose below for what a full `cargo test --workspace --lib --no-fail-fast` on this tree actually reports. This row can only be closed on the integrated tree, by whichever lane runs last."
---
