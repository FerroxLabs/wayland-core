...
criteria:
  ...
  - id: c7
    ...
    owner: core
    note: "CLOSED 2026-08-29 (lane f13-fin-hetzner-residuals). The verifier's refutation was correct and is now fixed at the wiring, not papered over in the note. THE FIX is one line in doctor::print_mcp_section: `wcore_mcp::malware_gate::install_mode(cfg.mcp.malware_gate)`, placed at the point the mode is READ FOR DISPLAY and therefore ahead of the probe block that spawns real stdio transports -- so the posture printed and the posture enforced cannot drift, and `--doctor` gets the mode without depending on AgentBootstrap::build it never reaches. install_mode is a one-shot OnceLock, so it cannot fight a later boot; nothing boots after --doctor. RED ARM, verbatim, with only the install line withdrawn (`git checkout -- doctor/mod.rs`, touched so cargo could not measure the old binary): `Probing config-declared MCP servers (connect-test)... / ~T probe-target failed: Transport error: MCP stdio server exited before responding` then `thread 'doctor_probe_mcp_launches_under_the_operators_gate_mode' (823191) panicked at crates/wcore-cli/tests/doctor_probe_mcp_honours_the_gate_mode.rs:134:5: / assertion `left == right` failed: --doctor --probe-mcp reached the MCP launch path with the operator's strict mode uninstalled, so it probed under the permissive default (FerroxLabs/wayland-core#354 c7) / left: Permissive / right: Strict`. NOTE WHAT THAT RED PROVES BEYOND THE ASSERTION: the probe really entered the launch path and really spawned -- the mock server line is in the output -- so this is the shipping non-session launch path and not a stand-in for it. GREEN on the same command with the line restored: 1 passed. WHY IT CANNOT PASS VACUOUSLY: install_mode is one-shot, so one process can observe exactly one installed mode; this is therefore its own test binary holding exactly ONE mode-installing test, and it asserts mode() == Permissive BEFORE the call (that is the defect state) and Strict after. CORRECTED 2026-08-30 (lane f13-mcp): the parenthetical used to say that pre-call assertion 'doubles as the permissive control'. It does not, and the claim was measured false. mode() cannot tell an uninstalled process from one that installed Permissive, because the uninstalled fallback IS Permissive -- so a doctor that ignored the config and installed a CONSTANT Strict passes this test. Measured: with `install_mode(cfg.mcp.malware_gate)` replaced by `install_mode(McpMalwareGateMode::Strict)`, this test PASSED (exit 0 for its binary) while c8's arm FAILED. The permissive direction is graded by c8, not here. An instrument-alive assertion on Config::resolve pins that the fixture really is strict, so a config that failed to load could not leave the process permissive and pass. The key is written into the GLOBAL config because c7 is about the OPERATOR's choice; an untrusted project file is narrowed by an allowlist and would grade something else."

  - id: c8
    text: "A PERMISSIVE operator's --doctor --probe-mcp probes under permissive, and does so because doctor read the config rather than because nothing installed a posture"
    state: met
    evidence: "test:crates/wcore-cli/tests/doctor_malware_gate_mode_permissive.rs::doctor_does_not_install_a_posture_the_operator_did_not_choose"
    owner: core
    note: "ADDED 2026-08-30 (lane f13-mcp). c7 grades the strict direction and cannot grade this one: `mode()` returns Permissive both for an uninstalled process and for an installed Permissive, because the uninstalled fallback IS Permissive, so `mode() == Permissive` is satisfied by a doctor that installs nothing at all -- which is the defect c7 exists to close. This arm is therefore written against a new `malware_gate::installed_mode() -> Option<McpMalwareGateMode>`, which returns None for an uninstalled process and Some(Permissive) only for a real install; that distinction is unwritable against `mode()`, which is why the public API is new rather than reused. Not a harmless over-tightening either: a doctor that installed a constant Strict would refuse every npx/uvx MCP server for a permissive operator the moment api.osv.dev were unreachable, which is exactly the offline regression the permissive default exists to avoid. Separate test binary because install_mode is a one-shot OnceLock and a process admits one install. The test opens with a NEGATIVE CONTROL asserting installed_mode() is None before doctor runs, so the Some(Permissive) it observes cannot be a leftover. RED ARM 2026-08-30, both arms run together, tree restored and touched after each, git status clean: (1) `install_mode(cfg.mcp.malware_gate)` -> `install_mode(McpMalwareGateMode::Strict)` -- THIS test FAILED ('doctor installed a posture the operator did not choose'), c7's test PASSED, which is the whole reason this criterion exists; (2) the install line deleted outright -- BOTH failed; (3) restored -- both passed."
---
