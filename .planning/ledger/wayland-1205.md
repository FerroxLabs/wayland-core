...
criteria:
  ...
  - id: c3
    ...
    owner: core
    note: "Both halves, run quoted. `cache list` over a store holding one legacy and one current session: `F23_CACHE=total sessions=1 ... uncached_equivalent_usd=unknown cost_truth=priced` -- the legacy zero is not summed in, it makes the store counterfactual unknown. `cache verify` exits 7 with `F23_CACHE=verify trustworthy=false cost_truth=priced saving_truth=unpriced legacy_schema=1 ...`; trustworthy is now `cost_truth().is_trustworthy() && !ledger.is_migrated()`, because a file whose field meanings were reinterpreted on read must be reported and not certified. The test carries a control: the current-schema session beside it still verifies `legacy_schema=none` at exit 0, so verify has not simply been broken for everything. Shown RED: dropping the `&& !ledger.is_migrated()` clause gives `assertion `left == right` failed: a migrated ledger is reported, never certified: left: "true" / right: "false"`."
  - id: c4
