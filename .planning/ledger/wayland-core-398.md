...
criteria:
  ...
  - id: c2
    ...
    state: not-met
    owner: core
    note: "RE-GRADED not-met 2026-08-31 (lane/f13-w3-vcs-regrade). Two defects in the GRADE, neither of them in the code.\n(1) ABSENT FILENAME. The criterion names `vfs_nested_named_store_deny.rs`. No such file exists in this tree (`ls crates/wcore-tools/tests/vfs_nested_named_store_deny.rs` -> No such file or directory); the only occurrences of that spelling anywhere in the repo are this ledger file and #394`s. What exists is `vfs_nested_store_deny.rs`. A criterion naming a file the tree does not have cannot be graded met AS WRITTEN. The discrepancy is recorded here and the filename is to be settled ON THE ISSUE, not quietly in this note.\n(2) THE EVIDENCE COULD NOT DISTINGUISH THE TWO OUTCOMES. It pointed at `vfs_guard_cost.rs::one_ordinary_path_guard_resolves_once_and_does_not_rescan`, which is a probe-COUNTING test: it asserts resolutions, scans, probe counts and `nested_walk_count`, and nothing else. It denies nothing and asserts no refusal, so it cannot tell `the refusals #390 bought stay bought` from `every refusal was reverted and the guard got cheap` -- which is exactly the reading this criterion exists to exclude. Pointer REMOVED rather than repointed: repointing it would grade a file the criterion does not name.\nTHE UNDERLYING PROPERTY IS TRUE AND IS MUTATION-SENSITIVE. Verified here on 0ed5d4707: `cargo test -p wcore-tools --test vfs_nested_store_deny` -> 8 passed, 0 failed, including both arms the criterion names (`a_vendored_gitfiles_object_store_is_refused` and `a_nested_alternates_borrow_is_refused_whatever_its_target_is_named`). The red arm recorded at #390 c1 stands: deleting arms 3+4 reddens 6 of the 8 tests in that file with the canary in the bytes. This is a POINTER defect, not a coverage defect -- move the pointer to `vfs_nested_store_deny.rs` on the issue and it closes.\n--- retained, verbatim ---\n`one_ordinary_path_guard_resolves_once_and_does_not_rescan` green: 1 resolution per guard, `scans` stays at 1 across 51 guards, warm probes exactly N*3 = 3 per guard. Unchanged from base. The COLD one-off moved and is stated rather than waved past: the first guard now costs 41 probes where it cost 17 on this fixture, because arm 4's walk runs once on the first guard. `nested_walks` is counted APART from `scans` so the two cannot be conflated, and the test asserts it stays at 1."
  - id: c3
