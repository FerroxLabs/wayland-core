...
criteria:
  ...
  - id: c4
    ...
    note: "RE-SCOPED AND SPLIT 2026-08-29. The old c4 read 'the remaining four bullets of this ticket's own Expected Behavior list are met' and handed the whole bag to flux. That was wrong, and the refutation is in a ticket the core lane itself filed: #1184 says in its own text that Flux declared the router-side truncate/stall causes fixed and live, that 'the remaining asks are harness-side, not Flux', and that #388's needs:flux label should be read as scoped to the failure-origin field and nothing else. The four bullets are now split out as c5 (met, core), c6 (not-met, core) and c7 (not-met, core). What genuinely cannot be decided from this repo stays here and is carried by #1184, which is open and needs:flux: a router rate limit and an upstream provider rate limit arrive as the same non-2xx from the same host, and x-flux-routed-model is absent on failed responses, so core would be guessing and presenting the guess as a classification RECONCILED ON MERGE WITH A COMPETING RE-GRADING, stated rather than reverted silently. The other lane re-owned this criterion TO CORE, keeping it as one bag and marking it not-met, on the correct finding that four of the Expected-Behavior bullets are agent-harness work in wcore-agent and not routing. That finding is not lost and is not overruled -- it is what c5, c6 and c7 below now carry, explicitly and separately, all three owned by core. The split is taken over the bag for the reason the release gate itself states: a partial is a ticket nobody split, and grading two shipped bullets, two unshipped core bullets and one genuinely unobservable flux bullet as a single not-met hides all three facts. Only the failure-ORIGIN half sits with flux here, on FerroxLabs/wayland#1184 -- open, area:flux + needs:flux, filed by the core lane itself, and confirmed to exist on merge."
  - id: c5
...
The competing re-grading from `lane/f13-fix-ledger-truth`'s successor kept c4 as one
core-owned bag rather than splitting it. Its substantive finding — that the remaining
bullets are harness behaviour in `wcore-agent`, not routing, and that the reporter
frames them that way — is ACCEPTED, and is what c6 and c7 record. What is rejected is
only the shape: one not-met covering two shipped bullets, two unshipped ones and one
unobservable one tells a reader none of the three.
