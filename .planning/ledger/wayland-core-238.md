...
criteria:
  ...
  - id: c4
    ...
    owner: core
    note: "RUN 2026-08-29 on SeanDesktop, and the previous owner was WRONG: this was recorded blocked-on-maintainer while the probe was already authored and the Windows box was reachable, so it was core work waiting on nobody. RECORD, verbatim from the run -- os build: Microsoft Windows [Version 10.0.26200.9168]; <dir>\\NUL kernel kind: CharDevice; <dir>\\NUL metadata is_file: None; bare NUL kernel kind: CharDevice; bare NUL metadata is_file: None; write to <dir>\\NUL reported: Ok; read back from <dir>\\NUL: Ok(0). Both facts the criterion names are now measured rather than assumed. (1) Bare NUL IS still a device on this build -- GetFileType on a live handle answers FILE_TYPE_CHAR, not FILE_TYPE_DISK -- so the #238 guard is refusing a device and not an addressable user file, which is the direction that would have destroyed data. (2) fs::metadata does NOT report is_file() true for it: it returns no verdict at all, which is the sharper reading of the same question -- the NonRegularFile arm is structurally BLIND here, so the NAME guard is the only thing standing between a Write and a silent discard. The behavioural half corroborates it end to end: the write reported Ok and the read back returned zero bytes. The probe carries its own liveness control (an ordinary file must measure FILE_TYPE_DISK and is_file() true) so it cannot pass by calling everything a device. Run as: cargo test -p wcore-tools --test windows_nul_device_probe -- --nocapture, in a worktree of this branch on D:. The build-20348 residual named in c5 is unchanged and still knowingly open."
  - id: c5
