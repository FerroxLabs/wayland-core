...
criteria:
  - id: c1
    ...
    owner: core
    note: "The first branch of the disjunction is taken: the session reports the real date, and the authoritative-date sentence is kept. refresh_current_date_line rewrites the ten date bytes after the FIRST `Current date: ` occurrence when they differ from today, and is applied at the one production site that puts the prompt on the wire (engine.rs, the `LlmRequest` built in `run`). It is a pure function of (prompt, today) and returns Cow::Borrowed within a day, so the cached prefix moves once per rollover rather than once per turn. A prompt with no recognisable date line is returned untouched. SINGLE-SITE claim re-derived exhaustively rather than by sampling: `grep -n 'LlmRequest {' crates/wcore-agent/src/engine.rs` returns SIX constructions (7516, 13458, 32168, 33120, 33258, 33824), not the five an earlier pass named. The sixth, 7516, is inside `prepare_recoverable_turn_for_test`, which carries `#[cfg(any(test, feature = \"test-utils\"))]` and builds its request with `..Default::default()`, so its `system` is empty and there is no date line in it to refresh. The conclusion is unchanged; the enumeration behind it now is."
  - id: c2
