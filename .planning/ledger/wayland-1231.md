...
criteria:
  ...
  - id: c6
    ...
    evidence: "test:crates/wcore-agent/tests/issue_1231_reasoning_recovery_test.rs::an_all_reasoning_turn_gives_the_user_an_answer"
    owner: core
    note: "MET. Re-run ON WINDOWS against a build carrying c2, which is what the criterion asks and what the previous grade correctly refused to claim from Linux evidence. HOST: SeanDesktop, Microsoft Windows [Version 10.0.26200.9168] -- the only Windows box. BUILD: `git clone --branch lane/f13-protocol --depth 1` to D:\\wcore-f13, cargo 1.95.0, `cargo build -p wcore-cli --bin wayland-core` -> Finished in 2m 42s. GUARD ON WINDOWS: `cargo nextest run -p wcore-agent --test issue_1231_reasoning_recovery_test` -> `6 tests run: 6 passed, 0 skipped`, all six including `an_all_reasoning_turn_gives_the_user_an_answer` and the three controls. AND THE REPORTER'S OWN CASE AGAINST A REAL MODEL, which is the half this row was really guarding -- not that the fix is platform-conditional (it is not: `take_captured()` plus a `ContentBlock::Text` push, no `#[cfg]` anywhere) but that the reporter's environment might produce a stream shape the Linux capture does not. It does not. The Windows binary was pointed at a real `qwen3:8b` over the OpenAI-compatible route (an SSH tunnel from the Windows box to the same hetzner Ollama the c1 fixture was captured from) and asked to keep its entire reply inside one `<thought>` block. The user got an answer: `[recovered from this model's reasoning - it emitted no answer outside its own tags]` followed by `The result of 2 + 2 is 4.`, then the info line naming the tags. Before c2 that turn was an accurate report of having no answer and nothing else. COMMAND: `wayland-core.exe --provider openai --base-url http://127.0.0.1:11439 --model qwen3:8b --api-key ollama 'Put your ENTIRE reply, including the final answer, inside a single <thought>...</thought> block. Write nothing outside the tags. Question: what is 2 + 2?'` with WAYLAND_HOME set to an isolated D:\\wcore-f13-home profile."
---
