...
criteria:
  ...
  - id: c6
    ...
    evidence: "test:crates/wcore-agent/tests/issue_1231_reasoning_recovery_test.rs::an_all_reasoning_turn_gives_the_user_an_answer"
    owner: core
    note: "NOT MET, and left not-met rather than graded on an adjacent property. The criterion is that the reporter's 2026-08-29 recurrence is re-run ON WINDOWS against a build carrying c2. Everything above was measured on Linux (hetzner). SeanDesktop is the only Windows box and this lane did not reach it, so nothing here is evidence for this row -- a green from a host that cannot exhibit the failure is not a pass, and #908 c1/c2 were already graded on Windows-adjacent evidence once. WHAT IS OWED, precisely: build this branch on Windows, run `cargo nextest run -p wcore-agent --test issue_1231_reasoning_recovery_test` there, and exercise the reporter's own case against a real model. NOTE ON DIFFICULTY, so the next lane does not re-derive it: nothing in the c2 fix is platform-conditional -- it is `take_captured()` plus a `ContentBlock::Text` push in the engine's turn loop, with no `#[cfg]` anywhere -- so the RISK this row is guarding is not that the fix behaves differently on Windows but that the reporter's environment produces a stream shape the Linux capture does not. That is what has to be checked, and it cannot be checked from here. --- MET 2026-08-31 BY THE RUN ITS OWN TEXT ASKS FOR, on real Windows. Host: SeanDesktop, Microsoft Windows 11 Pro build 26200 (the reporter is on Windows 11 Home; same family, and this is the only Windows box). Branch built there from this lane's own HEAD -- git rev-parse on the box matched the pushed sha, so the binary under test is this tree and not a neighbouring one. (1) THE SUITE, ON WINDOWS: cargo nextest run -p wcore-agent --test issue_1231_reasoning_recovery_test (plus the two #1266 relay files in the same invocation) -> 13 tests run: 13 passed, 0 skipped. (2) THE REPORTER'S OWN CASE AGAINST A REAL MODEL, which is the half the previous note said could not be checked from a Linux host. qwen3:8b pulled onto the Windows box's own Ollama and driven over the OpenAI-compatible route (POST /v1/chat/completions), temperature 0. FIRST, THE STREAM SHAPE WAS MEASURED RATHER THAN ASSUMED, because that is the exact risk this row guards -- whether the reporter's environment produces a shape the Linux capture does not. It does not: with the reporter's own <thought> tag the Windows content field carried '<thought>The capital of France is Paris.</thought>' INLINE -- 50 content chars, opener and closer both present in content -- the same class as the Linux fixture. A CONTROL RUN IN THE SAME PASS shows the measurement discriminates: without /no_think the same model routed its reasoning into the separate `reasoning` field (611 reasoning chars, 31 clean content chars, no tag in content), i.e. NOT this class, which is the Ollama-shim behaviour c1 already recorded. (3) THE PRODUCT, LIVE, ON WINDOWS: wayland-core.exe 0.13.11 built from this branch, run under an isolated WAYLAND_HOME with API_KEY / OPENAI_API_KEY / ANTHROPIC_API_KEY explicitly cleared from the environment first, against that endpoint. SUBJECT (the all-reasoning turn): the user GOT AN ANSWER -- 'The capital of France is Paris.' -- carrying '[recovered from this model's reasoning - it emitted no answer outside its own tags]' and the honest explanation beside it; exit 0. The reporter's symptom was 'not producing any response at all'; on their platform, against a real model emitting their own tag, they now get the reply. WRONG-REFUSAL CONTROL IN THE SAME SCRIPT: an ordinary turn on the same binary and the same endpoint returned 'The capital of France is Paris.' with NO recovery label at all, so the recovery does not fire on every turn and is not relabelling normal traffic. NOTED AND NOT CLAIMED AS FIXED HERE: the same runs surface unrelated existing notices on this host (no usable OS keyring, so crash replay protection is off; windows_job_object cannot enforce read-deny; qwen3:8b has no context-window entry). None of them touch this criterion and all are separately tracked."
---
