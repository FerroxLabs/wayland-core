...
criteria:
  ...
  - id: c4
    text: "The AppContainer route is recorded as foreclosed for this question in .planning/DECISIONS.md, so a future reversal of Q-391 inherits that bar instead of re-paying for it"
    state: met
    evidence: "file:.planning/DECISIONS.md:128:AppContainer is closed and must not be reopened"
    owner: core
    note: "CORRECTED 2026-08-30, and the correction is a REGRESSION THIS LANE INTRODUCED one round ago, found by running check-release-readiness LIVE rather than --offline. The row previously read `state: blocked, owner: maintainer, handoff: FerroxLabs/wayland-core#254` and this note asserted that #254 was `the open ticket that owns the Windows sandbox-backend question end to end`. THAT WAS FALSE WHEN IT WAS WRITTEN: #254 is a PR, CLOSED 2026-08-17, ten days before it was cited, and the live gate says so -- `HANDOFF: FerroxLabs/wayland-core#254 is CLOSED. A residual carried by a closed issue is untracked with extra steps.` The offline gate both this lane and the verifier ran resolves NO handoff targets, so neither run could see it; the lane asserted the ticket was open instead of checking, which is the same class of defect as a note claiming a scope the field does not carry. SEARCHED before re-pointing it: `gh issue list -R FerroxLabs/wayland-core --state open --search \"windows sandbox\"` and the same for `AppContainer`. Candidates and why each is wrong: #324 / #368 / #369 are AppContainer defects, i.e. tickets ON the route Q-391 forecloses, so handing a foreclosed-route constraint to them re-opens the thing the decision closed; #387 is an unrelated bash-resolution test break; #391 is THIS ticket, and a self-referential handoff is what this lane refused last round and will not now write. No open ticket owns the question because the question is DECIDED -- .planning/DECISIONS.md Q-391 is `NO -- and say so everywhere the product speaks`, with `do not reopen AppContainer` in the action column. So the criterion is not re-pointed at a substitute carrier: it is restated as the property that is true and gradeable TODAY -- the bar is RECORDED -- instead of a conditional on a branch nobody may take, hung off a dead link. What a future reader most needs to inherit is preserved exactly: the AppContainer route cost months and is closed; see DECISIONS.md line 81, `Why Q-391 is \"no\"`, which is this criterion`s evidence. CONSEQUENCE STATED RATHER THAN LEFT TO BE DISCOVERED: with c4 now met, every criterion in this file is met while #391 is still OPEN, so the LIVE check-criteria-ledger run reports a DIVERGENCE row for #391 that it did not report before. That is correct, and it is not a state chosen to dodge a gate. #391`s acceptance was decide-or-label; the decision was taken (Q-391 = NO), the gap is measured and pinned by a standing test, and the bar is recorded -- no engineering remainder is left to grade. The row means `this ticket is ready to close`, and closing an issue is a release/Sean action no lane may take. It joins 38 pre-existing rows of the same class. Original text, so nothing is lost:  this criterion is conditional on a decision that has been taken the other way (c1, Q-391 = no), so there is nothing for core to build and nothing for core to grade. It is kept rather than deleted because the condition is a real future branch, and the AppContainer bar is the part a future reader most needs to inherit -- that route cost months and is closed. #254 is the open ticket that owns the Windows sandbox-backend question end to end, so it is where this lands if the decision is ever reopened. RE-ANCHORED 2026-08-30 during the integ/f13 merge of lane/f13-keystone-1198, which closed FerroxLabs/wayland#1198 and made a bare file:<path>:<line> a REFUSED evidence token. This entry carried the bare DECISIONS.md:81, the section heading; the heading proves a section about Q-391 exists, not that the AppContainer route is recorded as FORECLOSED, which is what the criterion claims. It is now anchored on the sentence that states the bar -- `AppContainer is closed and must not be reopened (see core#254 history and the #389 note carrying the same bar)` -- inside the paragraph marked `Taken as no, for a reason that is already settled elsewhere`. Content re-anchor only; the criterion was not re-graded and DECISIONS.md was not edited."
---
