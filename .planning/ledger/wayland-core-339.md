...
criteria:
  ...
  - id: c2
    ...
    owner: core
    note: "MECHANISM SUBSTITUTED, deliberately. The criterion named symlink_metadata plus a canonical rel_to_root; the walk instead canonicalizes each entry and skips anything not under root_canonical (at_ref_resolve.rs:353-365), with a visited set. That permits an in-root dir symlink, which a bare symlink_metadata refusal would have broken - the fix the issue explicitly rejected. The escape is closed and tested; see c6 for the residual the lexical half leaves. SUBSTITUTE NOW GRADED 2026-08-29 (lane f13-n-atref): until then it was not. Both scope lines could be deleted with the whole suite still green - the cited evidence test survives them because its fixture target is NAMED .git-credentials, so is_secret_path alone satisfies it, and it grades the name denylist rather than the confinement this note points at. Two tests now grade the two lines separately. FILE arm: at_dir_never_inlines_an_ordinary_file_from_outside_the_workspace - an in-root link to an ordinary, non-denylisted file outside the workspace - RED ARM verbatim under 'if !scope.contains(&admitted.canonical)' -> 'if false': 'panicked at crates/wcore-cli/src/tui/commands/at_ref_resolve.rs:1398:9: the @dir walk inlined a file from outside the workspace through an in-root link'. DIR arm: at_dir_does_not_descend_through_a_link_out_of_the_workspace - its effect is not a leak (the file arm catches the bytes), it is that the walk never LEAVES the workspace, so the observable is the skip count: the link is ONE skipped entry, not the six files behind it. RED ARM under 'if !scope.contains(&canonical)' -> 'if false': the assertion goes Some(1) -> Some(6). Both carry an in-fixture control that the ordinary file in the same directory still attaches."
  - id: c3
  ...
  - id: c7
    text: "No @-surface read site opens a non-regular file, so a named pipe in the workspace cannot wedge a turn"
    state: met
    evidence: "test:crates/wcore-cli/src/tui/commands/at_ref_resolve.rs::the_at_dir_walk_does_not_block_on_a_fifo"
    owner: core
    note: "ADDED 2026-08-29 (lane f13-n-atref) for defect D7, found verifying c2. admit() OPENS the entry (same_file::Handle::from_path), and opening a FIFO blocks until a writer appears. resolve_file has always had an is_file() filter; the other two read sites on this surface did not, so @./ in any workspace containing a named pipe - build systems, editors and language servers all leave them - blocked forever inside a blocking syscall on the spawned turn task, where turn cancellation cannot reach it. PRE-EXISTING (git show 6d130a62^ has the same absent filter on the fs::read_to_string the walk used then), and c2 slightly widened it: admit() now runs BEFORE is_secret_path, so a FIFO named .env - which the pre-fix walk skipped by name without opening it - blocked too. Fixed at BOTH unguarded sites: walk_dir stats with fs::metadata (which follows the link and cannot block) before admit(), and read_guarded refuses a non-regular path. RED ARM, verbatim, on origin/integ/f13 before the fix: 'the @dir walk BLOCKED for 10s on a FIFO in the workspace - the turn is wedged, not slow' and, for the sibling site, 'read_guarded BLOCKED for 10s opening a FIFO - the same wedge as the @dir walk' (test:crates/wcore-cli/src/tui/commands/at_ref_resolve.rs::read_guarded_does_not_block_on_a_fifo). Each reddens again under a mutation of its own filter alone, so neither is carried by the other. The 10s is the tests' timeout; the block is unbounded."
