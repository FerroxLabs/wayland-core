...
criteria:
  ...
  - id: c2
    text: "The @dir walk judges every entry by the location it RESOLVES to - it canonicalizes the entry, refuses to leave the resolved root, and computes rel_to_root from the canonical path - rather than by the entry's spelling"
    state: met
