...
criteria:
  ...
  - id: c5
    ...
    state: met
    evidence: "test:crates/wcore-cli/src/main.rs::every_grant_refusal_exit_is_machine_readable_not_prose"
    owner: core
    note: "CLOSED 2026-08-30 by lane/f13-w3-teardown. `ProtocolEvent::GrantRefused { grant_id, surface, reason, detail }` is published at contract 1.23 (CONTRACT_MINOR 22 -> 23 plus GENERATOR_VERSION, which the generator REFUSED to bless without: `Desktop contract wire shape changed while the contract version stayed at 1.22 ... a host pinned to 1.22 has no way to learn the added wire types exist`). `surface` and `reason` are enums, not strings, so a host branches on the frame instead of on our English. THE SHAPE, NOT THE FOUR SITES: the ticket names four prose emissions, and repairing four `Info` calls would have been the enumeration failure this release keeps being refused for. Instead each handler now has exactly ONE refusal exit -- both causes are folded into the `Err` of a single `Result<_, (GrantRefusalReason, String)>` -- and `emit_grant_refusal` is that Err`s only consumer. The human line is DERIVED from the typed frame via `GrantSurface::refusal_prefix()`, so the refusal prose no longer exists as a literal anywhere in production main.rs (verified: 0 occurrences of `\"path grant refused` / `\"workspace capability grant refused` outside `#[cfg(test)]`). A fifth refusal cause therefore has nothing to copy: it must construct the Err, and the compiler routes it here. THE TWO CAUSES ARE KEPT APART -- LocalOptInRequired never reached the policy, PolicyRejected did -- because a single merged reason would satisfy \"a typed frame exists\" and still leave the host unable to tell \"turn on the flag\" from \"pick a different folder\". NEAR-MISS WORTH RECORDING, because it is #314`s own defect reproduced: the first attempt added the fixture in spec.rs but NOT the `wire!` declaration, and `wcore-contract check` reported `schema_digest is UNCHANGED ... no wire schema moved`. That is exactly #314 -- a variant with a fixture and no schema branch, unrepresentable in the published schema. The gate caught it; the WireSpec and the published-name entry were added and the digest then moved. c1`s exhaustive-match gate covers COMMANDS, not events, so it would not have caught this: that asymmetry is the residual noted below. Also required a `ProtocolEvent::GrantRefused` arm in crates/wcore-cli/src/tui/protocol_bridge.rs, found by `cargo check --workspace --all-targets` (a per-crate check misses downstream exhaustive matches). RED ARMS, tree committed first, each mutation `cargo check -p wcore-cli --tests` exit 0 so the red is an assertion and not a build error, and DISCRIMINATING -- each test reddened by a DIFFERENT mutation. M1: the `writer.emit(&ProtocolEvent::GrantRefused {..})` call deleted from `emit_grant_refusal` (executable, main.rs:4362, replaced by a compiling `let _ = (&grant_id, &surface, &reason);`). Verbatim: >>> assertion `left == right` failed: grant_path refused by the launcher must publish a typed frame carrying the host`s own grant_id / left: [] / right: [(Some(\"g-launcher\"), Path, LocalOptInRequired)] <<< Under M1 the negative control `a_granted_path_emits_no_refusal_frame` stayed PASS. M2 (always-fires): an unconditional GrantRefused emitted on the SUCCESS path of `emit_path_grant` (executable, main.rs:4326). Verbatim: >>> assertion `left == right` failed: a GRANTED folder must not publish a refusal / left: [(None, Path, PolicyRejected)] / right: [] <<< Under M2 `every_grant_refusal_exit_is_machine_readable_not_prose` stayed PASS. Restored with `git checkout --` and touched; blob identity verified `git hash-object` == `git rev-parse HEAD:<path>` == 257047f7a21776e42319efcc7ff1da063cea00bb. PREVIOUS NOTE KEPT, nothing lost: RE-OWNED TO CORE 2026-08-29.  RE-OWNED TO CORE 2026-08-29. The previous note said what is owed is a NEW Desktop-facing issue. Graded against the criterion text, the work is core's: both refusal sites are ProtocolEvent::Info with an empty msg_id and English prose in crates/wcore-cli/src/main.rs (path grant refused: the local launcher did not opt in ... at :4305, path grant refused: {error} at :4333, and the two capability-grant arms at :4389 and :4411). Making a refusal machine-readable is a typed variant in wcore-protocol plus a schema branch, a fixture and a manifest digest -- the same forward-additive shape wayland#1088 used for set_mode_refused, which core shipped alone at contract 21 -> 22. The maintainer decision this was parked on is already TAKEN as Q4 in .planning/DECISIONS.md. Desktop consuming the typed refusal is a real follow-on, but it is not this criterion, which is graded entirely on core's wire. ONE MORE CORRECTION, FROM THE AUDIT LANE AND KEPT: the maintainer label this entry used to carry was a parking artifact from when the decision was open. The decision has been TAKEN -- Q4 in .planning/DECISIONS.md, `YES, contract minor bump, with Desktop` -- and that row's Obliges cell pointed at FerroxLabs/wayland#1099, which is CLOSED and is a different subject. DECISIONS.md is corrected in this merge to name this criterion instead, so the decision row and the ledger row now point at each other."
---
