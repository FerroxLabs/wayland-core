...
Rendered 2026-08-29 18:46 UTC

## VERDICT: BLOCKED

**185 criteria block the 0.13.12 release.** Full list in §3.

| state | count | means |
|---|---:|---|
| DONE | 30 | met, evidence resolves, independently verified |
| CLAIMED | 209 | met but NOT yet independently verified — never report as done |
| OPEN | 194 | outstanding work |
| HANDOFF | 11 | another team's half, with a filed ticket carrying it |

### ⚠ 12 UNROUTED — nobody is doing these
...
| `c3` | core#367 | An integration run's failing-test SET is compared against a named allow-list, so `1 failed` can never be read as `the known 1 failed` |
