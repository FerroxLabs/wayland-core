...
- [ ] **REQ-native-r2**: Sandboxed process reads granted files / denied ungranted, grants revoked, isolation preserved (drop deny-only `SidsToDisable` in `CreateRestrictedToken`). — **OPEN (1 of 3 acceptance clauses proven).** PROVEN: `granted_path_is_readable_then_revoked` PASS 2.056s (target `windows-appcontainer-acl`) — grant ACE observed present on the host DACL during the run, exit 0 + MARKER read, ACE absent after; plus `one_execution_grant_never_leaks_to_another_identity` PASS 2.043s for the isolation clause. NOT PROVEN: the acceptance also requires "a genuine DENY ace still blocks" (`deny_ace_still_blocks_granted_read`) and "a file granted only to normal SIDs is still denied" (`normal_sid_only_grant_is_denied`). Neither is selected by any of the six proof targets, and their only runner — the `windows-live-acceptance` job — is gated `if: github.event.inputs.f20_candidate != 'true'` (`nightly-windows-soak.yml:239`), so it is **skipped by construction** in candidate mode. Confirmed skipped in run `30184651330`. These two tests have no observed green at the sealed SHA.
...
- [ ] **REQ-native-r8**: Structural guard preventing a native proof target mapping to a wrong-OS test. — **OPEN (rejection path never demonstrated).** The guard exists and RAN: `Assert-TargetOsGate` (`f20-native-windows-proof.ps1:113-162`) executes before every target, and the macOS mirror (`f20-native-macos-proof.sh:152-246`) before every macOS target. In run `30184651330` it admitted all four OS-specific Windows targets and both OS-specific macOS targets without firing. But the acceptance is "a native target mapped to the wrong-OS test **fails fast / is rejected by the guard**" — that direction has never been demonstrated. `scripts/f20-native-uat-proof.test.mjs` (34 cases) covers marker-log parsing, ordering, nonce drift and publication validation; it contains **no** wrong-OS-mapping case. Admitting correct mappings is not evidence that a wrong one is rejected.
...
**Closeout tally (2026-07-26):** **11 complete** — r1, r3, r4, r5, r6, r7, r9, r10, r11, r14, r15. **4 open** — r2 (2 of 3 deny clauses never run), r8 (guard rejection path never demonstrated), r12 (no fresh 20-16 / 20-17 at this SHA), r13 (no review artifact for this candidate; prior 2-claimed/1-recorded gap still measurable). Phase 20A's three Success Criteria are met by run `30184651330`; the four open requirements are acceptance detail the phase gate does not depend on, and none is a failing test — each is an **unexercised** path, not a red one.
...
| REQ-native-r2, r8, r12, r13 | Phase 20A | **Open** — unexercised acceptance clauses, not failing tests; see per-requirement notes above. Phase 20A closed COMPLETE on its three Success Criteria without them. |
...
*Last updated: 2026-07-26 — Phase 20A closeout: REQ-native r1–r15 dispositioned against dispatched run `30184651330` at sealed SHA `9821ef76`; 11 complete, 4 left open with the specific unmet clause named*
