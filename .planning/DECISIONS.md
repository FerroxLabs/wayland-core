...
| Q-389c2 / core#389 | Windows quarantine console: reach c1's property, or take c2's branch? | **TAKE c2 — LABEL the prompt. c1 is unreachable.** | `#389` measured both remedies foreclosed: reparenting is defeated by `AttachConsole(<pid>)` and a private console by `FreeConsole()` first. Windows has no session-leader equivalent, and the AppContainer route is closed by the decision above. PRODUCT COST, stated: this does NOT stop a determined child re-attaching to the operator's console — it only lets the operator ATTRIBUTE what appears. It also costs one unconditional stderr line per quarantine `git` spawn on Windows, which is noise on a non-interactive host, accepted because a notice that fires only when it guesses a human is watching is a notice that is absent exactly when it is wrong about that. Obliges: `#389` c1 stays OPEN and NOT-MET, and the residual pin measuring the bypass is KEPT, not deleted. **AMENDED 2026-08-31 (lane/f13-s3-wincon): THE DECISION STANDS — labelling is worth having either way — but the GROUND `c1 is unreachable` is REFUTED BY MEASUREMENT and must not be quoted as settled.** On real Windows 10.0.26200.9168, on the INTERACTIVE window station, a DETACHED_PROCESS child spawned with a LOW-INTEGRITY primary token reports `ATTACH_PARENT_PROCESS=FAILED` and `ATTACH_BY_EXPLICIT_PID=FAILED` with `SHARES_USER_CONSOLE=false` — BOTH spellings, which is exactly what c1 asks — against a MEDIUM-token control that still attaches through the same `CreateProcessAsUser` call (so the block is the INTEGRITY LEVEL, not the token API) and a wrong-refusal control in which a Low-integrity `git clone` still exits 0 with the tree on disk. The child's TOKEN, not its creation flags, decides this. It is NOT AppContainer and does not reopen it: a mandatory integrity label on an ordinary primary token is a different primitive from a capability sandbox. What keeps c1 open is COST, not reachability — the quarantine directory would need a Low mandatory label, the Low git chain failed to start on a SERVICE window station (every CI and headless run), and the change needs `CreateProcessAsUser`, replacing the spawn path `#393` just landed containment on. Numbers are on core#389 c1; the proposed repoint is to spawn Low ONLY when this process has a console |
...
| Q-406-postwalk / core#406 c1 | close the remaining half (a control dir created AFTER the walk declaring a borrow at a non-store-shaped target), or cut it out | **CUT IT OUT; c1 is repointed to the half that IS closed.** Not modelled, RUN: witnessing descended directories costs the warm gate-admitted guard 15 probes at 8 directories and 95 at 48 — slope **2.000 per directory**, cold 36 -> 44. On a 10,000-directory checkout that is ~20,000 `symlink_metadata` calls per file read, on a `SecretDenyFs` installed for **every sub-agent**. It is #398 c1's regression at twice the rate #398 itself reported, and it breaks #398 c1, #398 c2, #394 c2 and #390 c3, which all pin the warm ordinary guard at three probes. Four criteria and the product's read latency against one shape needing a checkout to appear mid-session AND declare an alternates borrow AND target a path with no store leaf. No bounded witness set can see it either — the borrow target is not an ancestor of the query path — so the price is inherent, not an implementation detail. Residual stays pinned live by `a_borrow_declared_by_a_control_dir_created_after_the_walk_is_still_admitted`, whose failure message instructs its own inversion. |
| Q-390-cost / core#390 c3 | keep does not get worse unqualified, or qualify it | **QUALIFY IT.** Everything moved down except one figure: warm guard unchanged at 3 probes with no nested checkout and 4 on a store-leaf-named path, slope still 0, cold first guard 41 -> 36, `vcs_content_stores` 17.000 -> 5.000 syscalls/dir. The exception is **+2 probes per vendored checkout** on the admit branch — the price of the half of #406 c1 that IS closed. Unqualified, this criterion can only be met by giving that refusal back. 2 probes per checkout, flat in directories, is worth a closed leak; #406 c2 already obliges anyone who moves the figure to re-grade it. |
| Q-pointer-repoints / core#394 c2+c4, core#398 c1+c2 | treat criteria naming absent tests as unfinished work, or repoint them | **REPOINT.** Each names a test or file that has NEVER been in this lineage — verified 0 hits with four known-positive controls green in the same call, so the zeros are real absences and not broken queries. #398 c1 additionally says to INVERT a slope-1.0 regression this tree never had: `merge-base --is-ancestor 875bf32cb HEAD` is NO, checked alongside three commits that are YES. A criterion pinned to an artefact the tree does not contain cannot pass however good the code is — a gate that cannot pass is worth as little as one that cannot fail. Every property was re-measured on the instrument that does the job here before any repoint was written. |
| Q-398-c5-carrier / core#398 c5 | grade the DENY_CACHE_MAX_DIRS branch here, or move it | **MOVE IT — core#413, 0.13.13.** `nested_stores_memoized` does not exist (0 hits, control 9). The surviving branch is `deny_cache`'s, inside `secret_deny_paths_for_backend`, a different memo #398 never touched, and reaching its false arm on the production path needs **100,001 directories** — not acceptable in a suite that runs on macOS and Windows CI. Left in c5 it was a residual pointed at a symbol nobody can find, which is not tracked but lost. |
| Q-389-c1-disposition / core#389 c1 | close the console bypass, or take the issue's own c2 branch | **TAKE c2; c1's residual goes to core#415 (0.13.13).** Measured on real Windows 26200 with the reproduction control alive in the SAME run, using `CONOUT$` + a canary (the process-list signal does NOT discriminate under ssh and reading it as if it did voided an earlier pass). Low IL **closes** the hole — both attach spellings `FAILED(5)`, `WROTE_TO_USER_CONSOLE=false` — and leaves the quarantine child unable to spawn **anything**: `cmd /c echo ok` exits `0xC0000142`, not merely git. Both boring causes eliminated: a Low-labelled `TMP`/`TEMP` and the full inherited environment, identical result each way. Separate window station and `JOB_OBJECT_UILIMIT_HANDLES` refuted with their own application proven, so those are real nulls. Mechanism (two independent reviews): since 1809 the console is `condrv.sys`, so USER32/win32k levers cannot reach a check gated on the child's token. Removing the feature to close a bypass the operator is now explicitly warned about is the worse trade. c4 repointed: there is no closure to assert, and inverting the pin would assert something measured false — the pin stays, RUNS, and asserts the live bypass plus the labelling. |
