#!/usr/bin/env bash
# w15/relay1352: name the direct caller of each relay overload and the
# aggregate counter at that moment, on one c8 16 MiB fast-reader batch.
set -u
W=/root/w15-relay1352/stacks2
...
bpftrace /root/w15-relay1352/relay-cancel-caller.bt > "$W/bpf.out" 2> "$W/bpf.err" &
BPF=$!
...
echo "attached_utc=$(date -u +%FT%TZ) loadavg=$(cut -d' ' -f1-3 /proc/loadavg)" >> "$ST"
python3 "$P" --binary "$BIN" --source "$SRC" --root "$W/armA-c8-16m-optional" --cycles 8 \
