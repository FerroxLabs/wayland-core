...
Plan: 17 of 18 — native path RED. Candidate `6937ef6` (working commit `be84bd2`). Next: native-repair successor (R1–R12) → fresh 20-16 (native NOT deferred) → 20-17 → 20-18
Status: Executing Phase 20
Last activity: 2026-07-23 — Native UAT (20-18) RUN on real Windows 11 hardware: RED. Root-caused the native breakage; the AppContainer sandbox boundary proven correct after two spike fixes (`storage.rs` `.write(true)`; drop deny-only SIDs in `CreateRestrictedToken`) — reads/grants/revokes/isolation all work. Full defect inventory + repair requirements R1–R12 in `.planning/inbox/native-uat-repair-BRIEF.md` (ingested 2026-07-23). The prior 20-16 CLEAN (2026-07-21) DEFERRED native by design; 20-18 is the gate that catches the RED, as intended. Next: native-repair successor (R1–R12) → fresh 20-16 (native NOT deferred) → 20-17 → 20-18 (Sean-authorized)
...
- **Native path RED (2026-07-23)** — SUPERSEDES the 2026-07-21 "20-17 BLOCKED / runners offline" note. 20-18 ran on real Windows 11 hardware and returned RED. The self-hosted msvc runner(s) (`ferrox-win-msvc` / `SEANDESKTOP`) were brought online and are AppContainer-capable; a macOS 8/8 proof ran on an ephemeral Scaleway Apple-silicon runner. Root cause: the 20-08 native-proof machinery (Windows AppContainer sandbox + Windows/macOS proof harness) was authored without ever running on the target OS, so native validation surfaces pre-existing breakage. Fully mapped + bounded in `native-uat-repair-BRIEF.md` (R1–R12): 2 proven-on-hardware sandbox fixes (uncommitted spikes), 1 `wcore-agent` compile bug, 2 test bugs, 2 harness targets wired to Linux-only Bubblewrap tests (real Windows Job-Object containment tests must be authored), + hygiene (stale `Cargo.lock`; tainted local tree to reset to pristine `be84bd2`; self-hosted-runner `runs-on`). Must be executed under a plan (native-repair successor), not improvised. The verifier-only-vs-writer gap in `scripts/f20-native-uat-proof.mjs` (needs an inline mode-0600 no-follow git-private request driver) is folded into that plan.
