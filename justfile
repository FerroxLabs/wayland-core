...
# ── Release board ─────────────────────────────────────────────────────────
# FerroxLabs/wayland#1272 c1: every issue in the blocking list on the board
# with a NAMED owner, and the board updated whenever the list changes.
# `check-release-board.py` fails on a missing row, a stale row, an `UNOWNED`
# row and a moved outstanding count, and reads owners out of each ledger's
# `owner:` rather than assigning them. `--self-test` proves it can still fail
# in each of those directions.
#
# NOT in `check-all`, on purpose: the board is regenerated by the integrator,
# so a lane that closes a criterion would go red on somebody else's file. The
# arm that decides is the ci.yml step of the same name, which runs where the
# blocking list actually moves. Regenerate with `--write` — integrator only.
#
# Run: `just release-board`
release-board:
    python3 scripts/check-release-board.py --self-test
    python3 scripts/check-release-board.py --offline

# Run: `just release-board-live` — corroborates `kind:` and handoffs on both
# trackers, so it can produce a SMALLER blocking set than the offline arm.
release-board-live:
    python3 scripts/check-release-board.py --self-test
