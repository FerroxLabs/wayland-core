...
**"NOT MEASURED" means not measured, and it is not a pass.** Six of the ten — Email, Signal,
iMessage, MS Teams, **Twilio SMS and WhatsApp** — have never had a replay driven at a real
destination. Their rows are derived: the adapter transmits nothing the destination is documented
...
**Telegram's row carries the same overstatement and is deliberately left standing** —
`lane/twilio-whatsapp-identity` owned SMS and WhatsApp and does not edit a row it did not
measure. Someone should fix it; the fix is to change `Yes` to
`NOT MEASURED at a real destination`, exactly as here.
