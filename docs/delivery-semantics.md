...
| **Matrix** | `PUT …/send/m.room.message/{txnId}` — the txn id *is* the idempotency slot | **exactly-once** | **retried** with the same key | one message; the homeserver returns the original `event_id` | **Yes — by the PRODUCT, against matrix.org, across a real `kill -9`.** See [§8](#8-the-matrix-row-driven-end-to-end-2026-07-30) |
...
the production factory. Matrix has since been driven end to end by the shipped binary — [§8](#8-the-matrix-row-driven-end-to-end-2026-07-30). That run is what makes the other rows interpretable: it is the known-positive proving
...
| Matrix | `wcore-channel-matrix/src/lib.rs:294` | `wcore-channel-matrix/src/rest.rs:63` `txn_id_for_key`, used `rest.rs:133-135`; bound by test `lib.rs:539`, and by the live wire capture in [§8](#8-the-matrix-row-driven-end-to-end-2026-07-30) |
...
## 8. The Matrix row, driven end to end (2026-07-30)
