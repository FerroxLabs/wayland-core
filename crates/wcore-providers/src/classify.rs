...
fn classify_by_status(status: u16) -> Option<FailoverReason> {
    // Semantic split: 503/529 are explicit "overloaded" signals; other 5xx
    // codes (500/502/504) are treated as transient timeouts so failover does a
    // fast retry / try the next provider rather than a long backoff.
    match status {
        ...
        // Other 5xx — server error; treat as Timeout so recovery is a fast
        // retry against the next provider rather than a long backoff.
...
fn classify_by_body(body: &str) -> Option<FailoverReason> {
    ...
    // Context overflow MUST be checked before generic Format patterns — it is
    // a distinct recovery class (compact / route to a larger-context model
    // rather than swap provider).
...
fn classify_by_sdk_code(code: &str) -> Option<FailoverReason> {
    ...
    match c.as_str() {
        ...
        // context_length_exceeded is its own recovery class (compact / pick a
        // larger-context model), NOT a Format error.
...
mod tests {
    ...
    fn sdk_code_openai_context_length_exceeded() {
        // context_overflow recovers via compaction or larger-context-model
        // routing, not by swapping providers.
        assert_eq!(
