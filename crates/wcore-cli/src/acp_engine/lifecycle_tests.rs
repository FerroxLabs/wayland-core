...
#[path = "../../tests/support/mock_llm.rs"]
mod child_mock_llm;
...
async fn close_waits_for_canonical_host_child_cancellation_and_parent_lease_release() {
    ...
    let mock = child_mock_llm::MockLlm::new()
        ...
        .slow_text("child must not complete normally", 60_000)
        .start()
        .await;
    ...
    let server = AcpServer::new().with_turn_engine(engine.clone());
    let id = create(&server).await;
    let frames = tokio::time::timeout(Duration::from_secs(20), async {
        server
            .send_message(MessageSendRequest {
                session_id: id.clone(),
    ...
    .await
    .expect("parent control deadline");
    ...
    let entered = tokio::time::timeout(Duration::from_secs(20), async {
        loop {
