...
// --- Disk-capacity admission: host-adaptive budget, measured refusal floor ---
//
// Every expected value below is a LITERAL. Re-deriving them from the
// production constants would make these tests agree with whatever formula the
// gate happens to hold, including the blanket one this fixed. A deliberate
// retune is expected to update these numbers.

/// Free space observed on a GitHub-hosted Windows runner, taken verbatim from
/// `.planning/evidence/win-hidden-fails/windows-hosted-panics.txt:3`. Using the
/// captured number rather than a round one keeps these tests bound to the host
/// that actually refused the documented quick-start.
const WINDOWS_HOSTED_AVAILABLE_BYTES: u64 = 30_758_768_640;
/// The pre-fix demand for four workers, verbatim from that same evidence line:
/// `4 * 8 GiB + 512 MiB`. Quoted so the non-vacuity check below is anchored to
/// the observed failure rather than to a re-derivation of it.
const PRE_FIX_FOUR_WORKER_DEMAND_BYTES: u64 = 34_896_609_280;
/// The pre-fix demand for five workers, verbatim from
/// `windows-hosted-panics.txt:13`.
const PRE_FIX_FIVE_WORKER_DEMAND_BYTES: u64 = 43_486_543_872;
/// Measured initial workspace (Git transfer closure plus logical checkout) of a
/// trivial one-commit repository, rounded up generously.
const ONE_COMMIT_REPO_BYTES: u64 = 4096;
/// Refusal floor the production rule must derive for [`ONE_COMMIT_REPO_BYTES`]:
/// `4096 * 4 + 512 MiB` = 16 384 + 536 870 912.
const ONE_COMMIT_FLOOR_BYTES: u64 = 536_887_296;
/// A checkout far larger than any share a 30 GB host can offer: 100 GiB.
const HUGE_CHECKOUT_BYTES: u64 = 107_374_182_400;
/// Refusal floor derived for [`HUGE_CHECKOUT_BYTES`]: `100 GiB * 4 + 512 MiB`.
const HUGE_CHECKOUT_FLOOR_BYTES: u64 = 430_033_600_512;
const ONE_TEBIBYTE: u64 = 1_099_511_627_776;

/// The two counts the CI failures were observed at must both be admitted on the
/// ~28.6 GiB free that a GitHub-hosted Windows runner offers, and each worker
/// must get a multi-gigabyte budget rather than a token one.
///
/// This is the test that proves the reported failure is fixed.
///
/// HOW THIS FAILS IF THE DEFECT RETURNS: in
/// `crates/wcore-swarm/src/worktree.rs`, replace
/// `let max_transaction_bytes = share_bytes.min(MAX_TRANSACTION_WORKSPACE_BYTES)...`
/// with the old blanket `MAX_TRANSACTION_WORKSPACE_BYTES.min(...)` and both
/// `assert_eq!`s below redden, because the budget stops tracking the host's
/// share. Restore the pre-fix gate whole — blanket budget plus its
/// `required > available` refusal — and the two `expect`s redden as well, with
/// exactly the `DispatchAdmission` the evidence file captured.
#[test]
fn derived_capacity_admits_the_documented_worker_counts_on_a_windows_hosted_runner() {
    ...
    let four = plan_workspace_capacity(4, ONE_COMMIT_REPO_BYTES, WINDOWS_HOSTED_AVAILABLE_BYTES, 0)
        .expect("the documented four-worker quick-start must be admitted with ~28.6 GiB free");
    assert_eq!(four.max_transaction_bytes, 7_555_474_432);

    let five = plan_workspace_capacity(5, ONE_COMMIT_REPO_BYTES, WINDOWS_HOSTED_AVAILABLE_BYTES, 0)
        .expect("five workers must be admitted with ~28.6 GiB free");
    assert_eq!(five.max_transaction_bytes, 6_044_379_545);

    for capacity in [four, five] {
        assert_eq!(capacity.available_bytes, WINDOWS_HOSTED_AVAILABLE_BYTES);
        assert_eq!(capacity.safety_margin_bytes, WORKSPACE_SAFETY_MARGIN_BYTES);
        assert_eq!(capacity.max_aggregate_bytes, MAX_AGGREGATE_WORKSPACE_BYTES);
    }
    // The gate's whole promise, checked against what it RETURNED: everything it
    // admitted, plus the margin, fits in the space it observed.
    assert!(
        four.max_transaction_bytes * 4 + four.safety_margin_bytes <= four.available_bytes,
        "the four-worker set does not fit in the space the gate proved"
    );
    assert!(
        five.max_transaction_bytes * 5 + five.safety_margin_bytes <= five.available_bytes,
        "the five-worker set does not fit in the space the gate proved"
    );
}

/// A host that is genuinely out of space must still be REFUSED, at any worker
/// count, and the refusal must print the numbers it actually used.
///
/// HOW THIS FAILS IF THE DEFECT RETURNS: delete the
/// `if share_bytes < floor_bytes { return Err(...) }` block in
/// `plan_workspace_capacity` (`crates/wcore-swarm/src/worktree.rs`) and every
/// `expect_err` here reddens — the gate would then shrink to fit any volume and
/// never refuse, which is exactly as worthless as never admitting. The zero-byte
/// case additionally reddens if the `saturating_sub` on `available_bytes`
/// becomes a wrapping or plain subtraction.
#[test]
fn derived_capacity_refuses_a_volume_that_is_genuinely_full() {
    // (workers, available, expected per-worker share printed in the refusal)
    for (workers, available, share) in [
        (5_usize, 1_000_000_000_u64, 92_625_817_u64),
        (1, 1_000_000_000, 463_129_088),
        (1, 0, 0),
    ] {
        let error = plan_workspace_capacity(workers, ONE_COMMIT_REPO_BYTES, available, 0)
            .expect_err("a full volume must refuse admission");
        let text = error.to_string();
        assert!(
            matches!(&error, SwarmError::DispatchAdmission(_)),
            "a capacity refusal must be a DispatchAdmission, got {text}"
        );
        // Diagnostic quality: the floor, the share it fell short of, the
        // measured checkout the floor came from, and the observed free space
        // must all be the values the arithmetic actually used.
        for expected in [
            ONE_COMMIT_FLOOR_BYTES.to_string(),
            share.to_string(),
            ONE_COMMIT_REPO_BYTES.to_string(),
            available.to_string(),
        ] {
            assert!(
                text.contains(&expected),
                "refusal for {workers} worker(s) on {available} bytes omitted {expected}: {text}"
            );
        }
    }
}

/// A volume with room to spare must still be REFUSED when the CHECKOUT itself
/// cannot fit in the per-worker share — a different failure mode from the empty
/// volume above, and both must stay reachable.
///
/// The contrast in the second half is what makes this non-vacuous: the SAME
/// host, same worker count, same everything but the measured checkout, is
/// admitted at the full 8 GiB ceiling.
///
/// HOW THIS FAILS IF THE DEFECT RETURNS: remove `measured_checkout_bytes` from
/// the `floor_bytes` derivation in `plan_workspace_capacity`
/// (`crates/wcore-swarm/src/worktree.rs`) — i.e. drop the
/// `.saturating_mul(WORKSPACE_GROWTH_MULTIPLIER)` term — and the floor collapses
/// to 512 MiB, the 100 GiB checkout is admitted onto a 30 GB volume, and the
/// `expect_err` reddens.
#[test]
fn derived_capacity_refuses_a_checkout_too_large_for_the_hosts_share() {
    let error = plan_workspace_capacity(1, HUGE_CHECKOUT_BYTES, WINDOWS_HOSTED_AVAILABLE_BYTES, 0)
        .expect_err("a 100 GiB checkout must not be admitted onto a 30 GB volume");
    let text = error.to_string();
    for expected in [
        HUGE_CHECKOUT_FLOOR_BYTES.to_string(),
        HUGE_CHECKOUT_BYTES.to_string(),
        // The share the host could actually offer: (30758768640 - 512 MiB) / 1.
        30_221_897_728_u64.to_string(),
    ] {
        assert!(
            text.contains(&expected),
    ...
    let admitted =
        plan_workspace_capacity(1, ONE_COMMIT_REPO_BYTES, WINDOWS_HOSTED_AVAILABLE_BYTES, 0)
            .expect("the same host must admit a trivial checkout");
    assert_eq!(
        admitted.max_transaction_bytes, MAX_TRANSACTION_WORKSPACE_BYTES,
        "the refusal above was the volume, not the measured checkout"
    );
}

/// A roomy host must still hand out the full 8 GiB ceiling — including for a
/// trivial repository — and the 64 GiB aggregate divided by the active worker
/// count must still bind above it.
///
/// The first assertion is the anti-regression one: budget IS the growth
/// observer's kill threshold, so a budget sized down to a trivial checkout
/// would kill any worker that runs a build. A startup refusal is recoverable;
/// a mid-flight kill is not.
///
/// HOW THIS FAILS IF THE DEFECT RETURNS: drop the
/// `.min(MAX_TRANSACTION_WORKSPACE_BYTES)` (or the
/// `.min(MAX_AGGREGATE_WORKSPACE_BYTES / active_workers)`) from the
/// `max_transaction_bytes` derivation in `crates/wcore-swarm/src/worktree.rs`
/// and the corresponding `assert_eq!` reddens with the host's whole share.
/// Sizing the budget from the measurement instead of the share reddens the
/// first `assert_eq!` with a megabyte-range budget.
#[test]
fn derived_capacity_hands_a_roomy_host_the_full_ceiling() {
    let trivial = plan_workspace_capacity(1, ONE_COMMIT_REPO_BYTES, ONE_TEBIBYTE, 0)
        .expect("a trivial repository must be admitted with 1 TiB free");
    assert_eq!(
        trivial.max_transaction_bytes, MAX_TRANSACTION_WORKSPACE_BYTES,
        "a roomy host stopped granting the full growth ceiling; a worker running a build \
         would now be killed mid-flight"
    );

    let large = plan_workspace_capacity(1, HUGE_CHECKOUT_BYTES, ONE_TEBIBYTE, 0)
        .expect("a 100 GiB checkout must be admitted with 1 TiB free");
    assert_eq!(
        large.max_transaction_bytes, MAX_TRANSACTION_WORKSPACE_BYTES,
        "the 8 GiB per-transaction ceiling stopped binding on a roomy host"
    );

    let fanned = plan_workspace_capacity(20, ONE_COMMIT_REPO_BYTES, ONE_TEBIBYTE, 0)
        .expect("twenty workers must be admitted with 1 TiB free");
    assert_eq!(
        fanned.max_transaction_bytes, 3_435_973_836,
        "the aggregate ceiling stopped dividing across active workers"
    );

    // Saturation safety: a measurement at `u64::MAX` must refuse, never wrap
    // into an admission.
    plan_workspace_capacity(1, u64::MAX, u64::MAX, 0)
        .expect_err("a saturating measurement wrapped into an admission");
}

/// Reservations already committed on disk must still be charged against both
/// the volume and the 64 GiB aggregate cap.
///
/// HOW THIS FAILS IF THE DEFECT RETURNS: delete the
/// `.saturating_sub(existing_reservation_bytes)` from the `share_bytes`
/// derivation, or the `if committed_bytes > MAX_AGGREGATE_WORKSPACE_BYTES`
/// block, in `plan_workspace_capacity`
/// (`crates/wcore-swarm/src/worktree.rs`) and the matching `expect_err`
/// reddens — a second dispatch would then be admitted on top of a swarm root
/// whose budget is already spoken for.
#[test]
fn existing_reservations_are_still_charged_against_volume_and_aggregate() {
    // 27 GiB already reserved leaves 246 173 696 bytes per worker for five
    // more, well under the 536 887 296-byte floor.
    let existing = 28_991_029_248_u64;
    let error = plan_workspace_capacity(
        5,
        ONE_COMMIT_REPO_BYTES,
        WINDOWS_HOSTED_AVAILABLE_BYTES,
        existing,
    )
    .expect_err("a nearly full swarm root must refuse five more workers");
    let text = error.to_string();
    for expected in [existing.to_string(), 246_173_696_u64.to_string()] {
        assert!(
            text.contains(&expected),
            "refusal omitted {expected}, so it did not charge the reservation: {text}"
        );
    }
    // The same request with nothing reserved is admitted, so the refusal above
    // is the reservation accounting and not the volume alone.
    plan_workspace_capacity(5, ONE_COMMIT_REPO_BYTES, WINDOWS_HOSTED_AVAILABLE_BYTES, 0)
        .expect("an empty swarm root must still admit five workers");

    // Aggregate cap: unlimited free space does not buy more than 64 GiB total.
    let error = plan_workspace_capacity(
        1,
        HUGE_CHECKOUT_BYTES,
        u64::MAX,
    ...
    .expect_err("the 64 GiB aggregate cap must refuse regardless of free space");
    assert!(
        error
            .to_string()
            .contains("aggregate workspace budget is already committed"),
        "{error}"
    );
}

/// End-to-end wiring: `workspace_capacity` must MEASURE the pinned checkout and
/// feed that measurement into the refusal floor.
///
/// # Why the wiring is proven through a REFUSAL
///
/// The measurement reaches the outcome only through `floor_bytes`, and
/// `floor_bytes` is only consulted on the refusal path — on a roomy host the
/// budget is the 8 GiB ceiling whether the checkout measured 4 KiB or nothing
/// at all. So an admission-only assertion could NOT redden if the
/// `transfer_closure_bytes` / `checkout_logical_bytes` calls were deleted, and
/// shipping one would be a test that merely looks like it covers the wiring.
///
/// This instead forces the refusal branch with an absurd worker count, then
/// requires the refusal to name the byte count this test measured
/// INDEPENDENTLY through the same two accessors. Deleting the measurement in
/// `WorktreeManager::workspace_capacity`
/// (`crates/wcore-swarm/src/worktree_manager.rs`) makes the message report a
/// zero-byte checkout and this reddens.
///
/// HOW THIS FAILS IF THE DEFECT RETURNS: remove the `transfer_closure_bytes` /
/// `checkout_logical_bytes` measurement, or the `plan_workspace_capacity(...)`
/// call, from `WorktreeManager::workspace_capacity` — the roomy-host assertion
/// reddens if the budget collapses to the measurement, and the refusal
/// assertion reddens if the measurement never reaches the gate.
///
/// Gated to Linux and Windows because it needs the shared git fixture
/// (`seal_init_repo`), which carries that gate; Windows is the platform whose
/// runner surfaced the defect, so it must run there.
#[cfg(any(target_os = "linux", windows))]
#[tokio::test]
async fn workspace_capacity_measures_the_pinned_checkout_and_feeds_the_refusal_floor() {
    let fixture = tempfile::tempdir().expect("fixture");
    let control = tempfile::tempdir().expect("orchestrator control root");
    seal_init_repo(fixture.path()).await;
    let manager =
    ...
    let measured = manager
        .transfer_closure_bytes(&head)
        .await
        .expect("closure measurement")
        + manager
            .checkout_logical_bytes(&head)
            .await
            .expect("checkout measurement");
    assert!(
        measured > 0,
        "the fixture repository measured zero bytes, so the wiring assertion below would be \
         vacuous"
    );

    // Roomy path: a trivial repository must NOT shrink the growth ceiling.
    // `derived_capacity_hands_a_roomy_host_the_full_ceiling` pins the exact
    // 8 GiB deterministically; this only has to prove the budget is not being
    // sized down to the measurement on a real host of unknown size.
    let capacity = manager
        .workspace_capacity(1)
        .await
        .expect("a one-commit repository must be admissible");
    assert!(
        capacity.max_transaction_bytes > 1024 * 1024 * 1024,
        "a one-commit repository was budgeted {} bytes on a host with {} free; the budget is \
         being sized from the measurement instead of the host's share",
        capacity.max_transaction_bytes,
        capacity.available_bytes
    );

    // Refusal path: an absurd worker count drives the per-worker share below
    // any possible floor on any host smaller than several petabytes, so the
    // refusal is reached deterministically — and it must carry the measured
    // byte count that only the production measurement can know.
    let error = manager
        .workspace_capacity(10_000_000)
        .await
        .expect_err("ten million workers must not be admitted");
    assert!(
        error.to_string().contains(&measured.to_string()),
        "the refusal reported a checkout this dispatch never measured; expected \
