...
#[test]
fn inventory_is_exactly_twenty_three_commands_and_fifty_nine_events() {
    assert_eq!(COMMAND_SPECS.len(), 23);
    assert_eq!(EVENT_SPECS.len(), 59);
    assert_eq!(
        COMMAND_SPECS
            .iter()
...
fn manifest_pins_generator_and_all_three_digests() {
    ...
    assert_eq!(manifest["events"].as_array().unwrap().len(), 59);
    assert_eq!(manifest["counts"]["commands"], 23);
    assert_eq!(manifest["counts"]["events"], 59);
