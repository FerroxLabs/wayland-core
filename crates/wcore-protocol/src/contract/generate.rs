...
//
// 19 -> 20: `mcp_ready_skip_annotation_v1` (wayland#605). `mcp_ready` gains an
// optional `already_connected` boolean. No field changes shape and nothing is
// newly required -- the flag is `skip_serializing_if`-omitted when false, so a
// real connect is byte-identical to what 1.19 emitted and a pinned host decodes
// it unchanged -- so `major` holds at 1. (The note above stops at 1.19 because
// it was written there; a pinned host now moves 1.16 -> 1.20.)
//
// The bump is what makes the flag mean anything. A skipped `add_mcp_server`
// re-add is acknowledged with an `mcp_ready` carrying the existing server's
// tools, which before this change was byte-identical to the receipt for a real
// reconnect. Because the annotation is absent-when-false, a host cannot
// feature-detect it by watching the wire: on 1.19 the field is absent on every
// frame, and on 1.20 it is absent on every real connect. "Absent" therefore
// answers a different question on each producer, and the version plus the
// capability are the only signals that separate them -- the same argument
// `session_persistence_v2` makes about a widened vocabulary a host already
