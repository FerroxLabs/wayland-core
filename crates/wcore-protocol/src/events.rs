...
/// The typed failure taxonomy, re-exported from `wcore-types`.
///
/// #1266 c3 moved the definition to [`wcore_types::failure::FailureCategory`]
/// so `SubAgentResult` (in `wcore-types`, below this crate) can carry a
/// child`s own category across the sub-agent relay. Re-exported here so
/// `wcore_protocol::events::FailureCategory` keeps resolving and the wire
/// representation is unchanged.
pub use wcore_types::failure::FailureCategory;
...
pub struct ErrorInfo {
    ...
    pub message: String,
    pub retryable: bool,
    /// wayland#1237. Additive: `serde(default)` means a payload written before
    /// this field existed still decodes, as `Unknown`; a host that has never
    /// heard of the key ignores it, exactly as the `error` event's published
    /// `additionalProperties: true` promises.
    #[serde(default)]
