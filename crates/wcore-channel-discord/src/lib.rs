...
impl Channel for DiscordChannel {
    ...
    /// This adapter DOES transmit the key, as the `nonce` field of the
    /// create-message body (see [`Self::post_message`]).
    ///
    /// This returned `false` until the nonce stopped being generated by a
    /// function whose documented purpose was to be "distinct across restarts".
    /// The default was HONEST while that was true — the adapter put a token on
    /// the wire that could not suppress the cross-restart replay, so claiming
    /// idempotency would have turned a visible duplicate into an invisible one.
    ///
    /// `discord_declares_idempotency_only_because_the_nonce_is_derived_from_the_key`
...
mod tests {
    ...
    /// this adapter. If the keyed path reverted to `next_nonce()` — whose whole
    /// documented property is being "distinct across restarts" — the claim
    /// would stand while the wire no longer backed it, and every such retry
    /// would post a second message. The mock matches the EXACT derived nonce,
    /// so that revert reddens here rather than only in a live channel.
    #[tokio::test]
    async fn discord_declares_idempotency_only_because_the_nonce_is_derived_from_the_key() {
        ...
        assert!(
            ...
            "discord claims it can deduplicate a replay"
        );
