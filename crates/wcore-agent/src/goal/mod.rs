...
//!
//! The reduced projection (`GoalState`, `GoalLifecycle`) lives beside the other
//! reduced state in `session_journal::model`, for the same reason
//! `ChildTransactionState` does: it is part of what the existing reducer folds,
//! not a second store.
...
pub use crate::session_journal::{GoalLifecycle, GoalState};
