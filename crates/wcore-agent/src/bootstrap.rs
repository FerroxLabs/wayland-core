...
impl AgentBootstrap {
    ...
    async fn build_scoped(mut self) -> anyhow::Result<BootstrapResult> {
        ...
        let hook_dispatcher: Option<Arc<dyn crate::hooks::HookDispatcher>> = if hook_dispatch_enabled
        ...
        let skill_router_to_install: wcore_skills::SkillRouter = {
            ...
            let candidate_names: Vec<String> = catalog.visible().into_iter().map(|r| r.name).collect();
