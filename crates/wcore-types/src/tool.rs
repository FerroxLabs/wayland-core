...
/// Reconcilers that certify one class of invocation could not have created an
/// external effect.
///
...
/// recognises, which is exactly what [`ToolEffectContract::reconciler`] already
...
/// operator, so a tool cannot mint recovery authority by inventing a reconciler
...
/// A shell command a static classifier proved cannot mutate anything: a single
...
/// which issues an HTTP GET with no body, and the search operation of the web
/// tool, whose backend contract is a query.
