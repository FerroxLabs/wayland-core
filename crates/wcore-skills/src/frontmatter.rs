...
pub fn parse_frontmatter(input: &str) -> ParsedMarkdown {
    parse_frontmatter_with_source(input, None)
}

/// Like [`parse_frontmatter`], but records the source file path so that any
/// YAML parse failure can name the offending skill in its warning. Production
/// loaders pass `Some(path)`; pass `None` when no path is available.
pub fn parse_frontmatter_with_source(input: &str, source_path: Option<&str>) -> ParsedMarkdown {
    match extract_frontmatter_bounds(input) {
        Some((yaml_text, content)) => {
            let frontmatter = parse_yaml_with_fallback(yaml_text, source_path);
            ParsedMarkdown {
                frontmatter,
...
pub fn parse_skill_fields(
    ...
    let artifacts = parse_artifacts_spec(frontmatter.artifacts.as_ref());
...
/// W4 X4: parse the `artifacts:` YAML value into a typed list.
///
/// Malformed YAML falls back to an empty list — matches the rest of
/// `frontmatter.rs`'s "fall back to empty" posture for unknown shapes.
fn parse_artifacts_spec(raw: Option<&serde_yaml::Value>) -> Vec<crate::types::ArtifactSpec> {
    let Some(v) = raw else {
        return Vec::new();
    };
    serde_yaml::from_value::<Vec<crate::types::ArtifactSpec>>(v.clone()).unwrap_or_default()
}
...
fn parse_yaml_with_fallback(yaml_text: &str, source_path: Option<&str>) -> FrontmatterData {
    // Include the source file path in diagnostics so a malformed skill can be
    // identified at a glance (the YAML error's line/column is relative to the
    // frontmatter block, i.e. after the opening `---`).
    let where_ = match source_path {
        Some(p) => format!(" ({p})"),
        None => String::new(),
    };
    ...
    match serde_yaml::from_str::<FrontmatterData>(yaml_text) {
        Ok(data) => return data,
        Err(e) => {
            eprintln!("skills: frontmatter first-pass parse failed{where_}: {e}");
        }
    }
    ...
    match serde_yaml::from_str::<FrontmatterData>(&fixed) {
        ...
        Err(e) => {
            eprintln!(
                "skills: frontmatter second-pass parse failed{where_}: {e}; returning empty frontmatter"
            );
