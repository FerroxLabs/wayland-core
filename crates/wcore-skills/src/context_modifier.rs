...
// Re-export from wcore-types so callers can use a single import path
pub use wcore_types::skill_types::{ContextModifier, effort_to_string};
...
mod tests {
    ...
    use wcore_types::skill_types::EffortLevel;
    ...
    fn test_from_skill_model_override() {
        let skill = make_skill(Some(ANTHROPIC_OPUS), None, vec![]);
    ...
    fn test_from_skill_all_overrides() {
        ...
        let m = from_skill(&skill).unwrap();
        assert_eq!(m.model.as_deref(), Some(OPENAI_GPT4O));
