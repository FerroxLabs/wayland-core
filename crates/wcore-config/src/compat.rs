...
mod prompt_cache_expected_pinning_tests {
    ...
    fn only_justified_presets_expect_a_served_prompt_cache() {
        // The table is read off the REAL preset constructors (via
        // `compat_defaults_for`) rather than a hand-built `ProviderCompat`: a
        // struct literal cannot observe struct-update inheritance, which is
        // exactly how `prompt_cache_expected` leaked from `openai_defaults()`
        // into every openai-compat preset and from `anthropic_defaults()` into
        // Vertex and MiniMax.
        //
        // Which providers get checked is `ProviderType::ALL`, generated by the
        // macro that declares the variants, so a new provider is in this loop
        // the moment it exists. It replaces a hand-written `[ProviderType; 23]`
        // whose staleness guard — `assert_eq!(ALL_PROVIDER_TYPES.len(), 23)` —
        // compared a fixed-size array's length to its own literal, so it could
        // never fire for the one case it was written to catch: a variant nobody
        // appended.
        //
        // Duplication is the one property generation does NOT make
        // unrepresentable, and a duplicate row means some other row is
        // silently unchecked. So assert it here, at runtime.
        let mut seen: std::collections::HashSet<String> = std::collections::HashSet::new();
        for provider in ProviderType::ALL {
            assert!(
                seen.insert(format!("{provider:?}")),
                "ProviderType::ALL lists {provider:?} twice"
            );
        }
