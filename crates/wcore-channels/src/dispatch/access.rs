...
///
/// A field belongs in the shape iff it decides **who is admitted, or what an
/// admitted principal can trigger without addressing the bot**:
///
...
///
/// `tools` / `tool_workspace_root` (authority granted to an admitted turn, not
/// admission), `ack` (how the bot signals it is working), and the two
/// session-shaping flags. None of them change who reaches the agent, and every
/// field in here is a field whose edit costs the operator a re-acknowledgement
/// — so the line is drawn at what the admission path actually consults, not at
/// "everything that looks security-adjacent".
pub const SHAPE_FIELDS: [&str; 7] = [
    "enabled",
    "dm",
    "dm_allowlist",
    "group",
